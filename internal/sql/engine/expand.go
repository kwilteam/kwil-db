...
func (c *Engine) expand(qc *QueryCatalog, raw *ast.RawStmt) ([]source.Edit, error) {
...
func (c *Engine) quoteIdent(ident string) string {
	if c.IsReservedKeyword(ident) {
		switch c.Kind() {
	...
	if c.Kind() == core.EnginePostgreSQL {
...
func (c *Engine) expandStmt(qc *QueryCatalog, raw *ast.RawStmt, node ast.Node) ([]source.Edit, error) {
