...
import (
	...
	"github.com/kwilteam/kwil-db/internal/sql/astutils"
	"github.com/kwilteam/kwil-db/internal/sql/core"
	"github.com/kwilteam/kwil-db/internal/sql/rewrite"
	"github.com/kwilteam/kwil-db/internal/sql/source"
	"github.com/kwilteam/kwil-db/internal/sql/validate"
)
...
func (c *Engine) parseQuery(stmt ast.Node, src string) (*Query, error) {
	...
	if err := validate.FuncCall(c.Catalog, raw, true); err != nil {
		return nil, err
	}

	raw, namedParams, edits := rewrite.NamedParameters(c.Kind(), raw, numbers, dollar)
	...
	if c.Kind() == core.EngineMySQL || !dollar {
	...
	if expanded != rawSQL {
		if _, err := c.Parse(strings.NewReader(expanded)); err != nil {
