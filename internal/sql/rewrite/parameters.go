...
import (
	...
	"github.com/kwilteam/kwil-db/internal/sql/named"
...
func NamedParameters(kind core.EngineKind, raw *ast.RawStmt, numbs map[int]bool, dollar bool) (*ast.RawStmt, *named.ParamSet, []source.Edit) {
	foundFunc := astutils.Search(raw, named.IsParamFunc)
	foundSign := astutils.Search(raw, named.IsParamSign)
	hasNamedParameterSupport := kind != core.EngineMySQL
	...
	node := astutils.Apply(raw, func(cr *astutils.Cursor) bool {
		...
		case named.IsParamFunc(node):
			...
			var replace string
			if kind == core.EngineMySQL || !dollar {
				replace = "?"
			} else {
