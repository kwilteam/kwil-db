...
func (i *inspect) InspectRealm(ctx context.Context, opts *schema.InspectRealmOption) (*schema.Realm, error) {
	...
	schema.LinkSchemaTables(schemas)
	return schema.ExcludeRealm(r, opts.Exclude)
}
...
func (i *inspect) InspectSchema(ctx context.Context, name string, opts *schema.InspectOptions) (s *schema.Schema, err error) {
	...
	case n == 0:
		return nil, &sqlx.NotExistError{Err: fmt.Errorf("postgres: schema %q was not found", name)}
	case n > 1:
...
func (i *inspect) fks(ctx context.Context, s *schema.Schema) error {
	...
	defer rows.Close()
	if err := schema.SchemaFKs(s, rows); err != nil {
		return fmt.Errorf("postgres: %w", err)
	}
	return rows.Err()
}
...
func canConvert(t *schema.ColumnType, x string) (string, bool) {
	...
	case *ArrayType, *schema.BinaryType, *schema.JSONType, *NetworkType, *schema.StringType, *schema.DateTimeType, *UUIDType, *XMLType:
		return q, true
	}
	return "", false
}
