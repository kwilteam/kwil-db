...
func parseRows(table string) *sqlmock.Rows {
	var (
		nc    int
		rows  *sqlmock.Rows
...
`))
				m.ExpectQuery(sqlx.Escape(`SELECT enumtypid, enumlabel FROM pg_enum WHERE enumtypid IN ($1, $2)`)).
...
`))
			...
			expect: func(require *require.Assertions, t *schema.Table, err error) {
				...
				require.EqualValues([]*schema.Column{
					...
					{Name: "c10", Type: &schema.ColumnType{Raw: "date", Type: &schema.DateTimeType{T: "date"}}},
					{Name: "c11", Type: &schema.ColumnType{Raw: "time with time zone", Type: &schema.DateTimeType{T: "time with time zone", Precision: p(0)}}},
					...
					{Name: "c22", Type: &schema.ColumnType{Raw: "ARRAY", Nullable: true, Type: &ArrayType{Type: &schema.IntegerType{T: "integer"}, T: "integer[]"}}},
					{Name: "c23", Type: &schema.ColumnType{Raw: "USER-DEFINED", Nullable: true, Type: &UserDefinedType{T: "ltree"}}},
					{Name: "c24", Type: &schema.ColumnType{Raw: "state", Type: &schema.EnumType{T: "state", Values: []string{"on", "off"}, Schema: t.Schema}}},
					{Name: "c25", Type: &schema.ColumnType{Raw: "timestamp without time zone", Type: &schema.DateTimeType{T: "timestamp without time zone", Precision: p(4)}}, Default: &schema.RawExpr{X: "now()"}},
					{Name: "c26", Type: &schema.ColumnType{Raw: "timestamp with time zone", Type: &schema.DateTimeType{T: "timestamp with time zone", Precision: p(6)}}},
					{Name: "c27", Type: &schema.ColumnType{Raw: "time without time zone", Type: &schema.DateTimeType{T: "time without time zone", Precision: p(6)}}},
...
users           | idx1            | btree       |             | f        | f       | f      |                 | (id <> NULLable::integer) | "left"((c11)::text, 100)  | t    | t           | f          |           |
...
`))
			...
			expect: func(require *require.Assertions, t *schema.Table, err error) {
				...
				indexes := []*schema.Index{
					...
					{Name: "idx1", Table: t, Attrs: []schema.Attr{&IndexType{T: "btree"}, &IndexPredicate{P: `(id <> NULLable::integer)`}}, Parts: []*schema.IndexPart{{SeqNo: 1, X: &schema.RawExpr{X: `"left"((c11)::text, 100)`}, Desc: true, Attrs: []schema.Attr{&IndexColumnProperty{NullsFirst: true}}}}},
...
func TestDriver_InspectPartitionedTable(t *testing.T) {
	...
	drv, err := Open(db)
	require.NoError(t, err)
	mk.ExpectQuery(sqlx.Escape(fmt.Sprintf(schemasQueryArgs, "= CURRENT_SCHEMA()"))).
		WillReturnRows(parseRows(`
   schema_name
--------------------
public
`))
	m.ExpectQuery(sqlx.Escape(fmt.Sprintf(tablesQuery, "$1"))).
		WithArgs("public").
		WillReturnRows(parseRows(`
...
`))
	m.ExpectQuery(sqlx.Escape(fmt.Sprintf(columnsQuery, "$2, $3, $4"))).
...
`))
	m.ExpectQuery(sqlx.Escape(fmt.Sprintf(indexesQuery, "$2, $3, $4"))).
	...
	m.ExpectQuery(sqlx.Escape(fmt.Sprintf(fksQuery, "$2, $3, $4"))).
	...
	m.ExpectQuery(sqlx.Escape(fmt.Sprintf(checksQuery, "$2, $3, $4"))).
...
func TestDriver_Realm(t *testing.T) {
	...
	drv, err := Open(db)
	require.NoError(t, err)
	mk.ExpectQuery(sqlx.Escape(schemasQuery)).
		WillReturnRows(parseRows(`
   schema_name
--------------------
test
public
`))
	m.ExpectQuery(sqlx.Escape(fmt.Sprintf(tablesQuery, "$1, $2"))).
	...
	mk.ExpectQuery(sqlx.Escape(fmt.Sprintf(schemasQueryArgs, "IN ($1, $2)"))).
		WithArgs("test", "public").
		WillReturnRows(parseRows(`
   schema_name
--------------------
...
func (m mock) version(version string) {
	m.ExpectQuery(sqlx.Escape(paramsQuery)).
		WillReturnRows(parseRows(`
  setting
------------
 ` + version + `
