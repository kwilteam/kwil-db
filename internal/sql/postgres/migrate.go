...
func (p *planApply) PlanChanges(ctx context.Context, name string, changes []schema.SchemaChange, opts ...schema.PlanOption) (*schema.Plan, error) {
	s := &state{
		conn: p.conn,
		Plan: schema.Plan{
			Name:          name,
...
func (p *planApply) ApplyChanges(ctx context.Context, changes []schema.SchemaChange, opts ...schema.PlanOption) error {
	return schema.ApplyChanges(ctx, changes, p, opts...)
}
...
func (s *state) plan(ctx context.Context, changes []schema.SchemaChange) error {
	if s.SchemaQualifier != nil {
		if err := schema.CheckChangesScope(changes); err != nil {
			return err
		}
	}
	planned := s.topLevel(changes)
	planned, err := schema.DetachCycles(planned)
	if err != nil {
		return err
	}
...
func (s *state) topLevel(changes []schema.SchemaChange) []schema.SchemaChange {
	planned := make([]schema.SchemaChange, 0, len(changes))
	for _, c := range changes {
		switch c := c.(type) {
		case *schema.AddSchema:
			b := s.Build("CREATE SCHEMA")
			if schema.Has(c.Extra, &schema.IfNotExists{}) {
				b.P("IF NOT EXISTS")
			}
			b.Ident(c.S.Name)
			s.append(&schema.Change{
				Cmd:     b.String(),
				Source:  c,
		...
		case *schema.DropSchema:
			...
			if schema.Has(c.Extra, &schema.IfExists{}) {
				b.P("IF EXISTS")
			}
...
func (s *state) addTable(ctx context.Context, add *schema.AddTable) error {
	...
	if schema.Has(add.Extra, &schema.IfNotExists{}) {
		b.P("IF NOT EXISTS")
	}
	b.Table(add.T)
	b.Wrap(func(b *schema.Builder) {
		b.MapComma(add.T.Columns, func(i int, b *schema.Builder) {
	...
	if p := (Partition{}); schema.Has(add.T.Attrs, &p) {
...
func (s *state) dropTable(drop *schema.DropTable) {
	...
	if schema.Has(drop.Extra, &schema.IfExists{}) {
		b.P("IF EXISTS")
	}
	b.Table(drop.T)
	s.append(&schema.Change{
...
func (s *state) modifyTable(ctx context.Context, modify *schema.ModifyTable) error {
	...
	for _, change := range skipAutoChanges(modify.Changes) {
		...
		case *schema.AddIndex:
			if c := (schema.Comment{}); schema.Has(change.I.Attrs, &c) {
		...
		case *schema.ModifyIndex:
			...
			if change.Change.Is(schema.ChangeComment) {
				from, to, err := commentChange(schema.CommentDiff(change.From.Attrs, change.To.Attrs))
				if err != nil {
					return err
				}
		...
		case *schema.AddColumn:
			...
			if c := (schema.Comment{}); schema.Has(change.C.Attrs, &c) {
...
func (s *state) alterTable(t *schema.Table, changes []schema.SchemaChange) error {
	var (
		reverse    []schema.SchemaChange
		reversible = true
	)
	build := func(alter *alterChange, changes []schema.SchemaChange) (string, error) {
		b := s.Build("ALTER TABLE").Table(t)
		err := b.MapCommaErr(changes, func(i int, b *schema.Builder) error {
...
// alterChange describes an alter table schema.SchemaChange where its main command
// can be supported by additional statements before and after it is executed.
type alterChange struct {
	main          *schema.Change
	before, after []*schema.Change
}
...
func (s *state) alterColumn(b *schema.Builder, alter *alterChange, t *schema.Table, c *schema.ModifyColumn) error {
	for k := c.Change; !k.Is(schema.NoChange); {
		...
		case k.Is(schema.ChangeGenerated):
			if schema.Has(c.To.Attrs, &schema.GeneratedExpr{}) {
...
func (s *state) alterType(b *schema.Builder, alter *alterChange, t *schema.Table, c *schema.ModifyColumn) error {
	...
	case fromHas && !toHas:
		...
		alter.after = append(alter.after, &schema.Change{
			Source:  c,
	...
	case !fromHas && toHas:
		...
		alter.before = append(alter.before, &schema.Change{
			Source:  c,
	...
	if collate := (schema.Collation{}); schema.Has(c.To.Attrs, &collate) {
...
func (s *state) addComments(t *schema.Table) {
	...
	if schema.Has(t.Attrs, &c) && c.Text != "" {
		s.append(s.tableComment(t, c.Text, ""))
	}
	for i := range t.Columns {
		if schema.Has(t.Columns[i].Attrs, &c) && c.Text != "" {
			s.append(s.columnComment(t, t.Columns[i], c.Text, ""))
		}
	}
	for i := range t.Indexes {
		if schema.Has(t.Indexes[i].Attrs, &c) && c.Text != "" {
			s.append(s.indexComment(t, t.Indexes[i], c.Text, ""))
		}
	}
}

func (s *state) tableComment(t *schema.Table, to, from string) *schema.Change {
...
func (s *state) columnComment(t *schema.Table, c *schema.Column, to, from string) *schema.Change {
...
func (s *state) indexComment(t *schema.Table, idx *schema.Index, to, from string) *schema.Change {
...
func (s *state) column(b *schema.Builder, t *schema.Table, c *schema.Column) error {
	...
	switch hasI, hasX := schema.Has(c.Attrs, &Identity{}), schema.Has(c.Attrs, &schema.GeneratedExpr{}); {
	case hasI && hasX:
...
func (s *state) columnDefault(b *schema.Builder, c *schema.Column) {
...
func (s *state) indexParts(b *schema.Builder, parts []*schema.IndexPart) {
	b.Wrap(func(b *schema.Builder) {
		b.MapComma(parts, func(i int, b *schema.Builder) {
			switch part := parts[i]; {
			case part.C != nil:
				b.Ident(part.C.Name)
			case part.X != nil:
...
func (s *state) partAttrs(b *schema.Builder, p *schema.IndexPart) {
	if p.Desc {
		b.P("DESC")
	}
...
func (s *state) index(b *schema.Builder, idx *schema.Index) {
	// Avoid appending the default method.
	if t := (IndexType{}); schema.Has(idx.Attrs, &t) && strings.ToUpper(t.T) != IndexTypeBTree {
		b.P("USING", t.T)
	}
	s.indexParts(b, idx.Parts)
	if c := (IndexInclude{}); schema.Has(idx.Attrs, &c) {
		b.P("INCLUDE")
		b.Wrap(func(b *schema.Builder) {
			b.MapComma(c.Columns, func(i int, b *schema.Builder) {
...
func (s *state) fks(b *schema.Builder, fks ...*schema.ForeignKey) {
	b.MapComma(fks, func(i int, b *schema.Builder) {
		fk := fks[i]
		if fk.Symbol != "" {
		...
		b.P("FOREIGN KEY")
		b.Wrap(func(b *schema.Builder) {
			b.MapComma(fk.Columns, func(i int, b *schema.Builder) {
				b.Ident(fk.Columns[i].Name)
			})
		})
		b.P("REFERENCES").Table(fk.RefTable)
		b.Wrap(func(b *schema.Builder) {
			b.MapComma(fk.RefColumns, func(i int, b *schema.Builder) {
...
func (s *state) append(c ...*schema.Change) {
	s.Changes = append(s.Changes, c...)
}

// Build instantiates a new builder and writes the given phrase to it.
func (s *state) Build(phrases ...string) *schema.Builder {
	b := &schema.Builder{QuoteChar: '"', Schema: s.SchemaQualifier}
	return b.P(phrases...)
}

// skipAutoChanges filters unnecessary changes that are automatically
// happened by the database when ALTER TABLE is executed.
func skipAutoChanges(changes []schema.SchemaChange) []schema.SchemaChange {
	var (
...
func commentChange(c schema.SchemaChange) (from, to string, err error) {
	switch c := c.(type) {
	case *schema.AddAttr:
...
func (s *state) createDropEnum(sc *schema.Schema, e *schema.EnumType) (string, string) {
	...
	return s.Build("CREATE TYPE").
			P(name, "AS ENUM").
			Wrap(func(b *schema.Builder) {
				b.MapComma(e.Values, func(i int, b *schema.Builder) {
