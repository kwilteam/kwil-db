...
func TestPlanChanges(t *testing.T) {
	tests := []struct {
		...
		options  []schema.PlanOption
		mock     func(mock)
		wantPlan *schema.Plan
		wantErr  bool
	}{
		{
			changes: []schema.SchemaChange{
				&schema.AddSchema{S: schema.New("test"), Extra: []schema.SchemaClause{&schema.IfNotExists{}}},
				&schema.DropSchema{S: schema.New("test"), Extra: []schema.SchemaClause{&schema.IfExists{}}},
				&schema.DropSchema{S: schema.New("test"), Extra: []schema.SchemaClause{}},
			},
			wantPlan: &schema.Plan{
				Reversible:    false,
				Transactional: true,
		...
		},
		{
			...
			wantPlan: &schema.Plan{
				Reversible:    true,
				Transactional: true,
				Changes:       []*schema.Change{{Cmd: `CREATE SCHEMA "test"`, Reverse: `DROP SCHEMA "test" CASCADE`}}},
		},
		{
			changes: []schema.SchemaChange{
			...
			wantPlan: &schema.Plan{
				Transactional: true,
				Changes:       []*schema.Change{{Cmd: `DROP SCHEMA "atlas" CASCADE`}},
			},
		},
		{
			changes: []schema.SchemaChange{
				&schema.AddTable{
					T: &schema.Table{
						...
						Columns: []*schema.Column{
							...
							{Name: "text", Type: &schema.ColumnType{Type: &schema.StringType{T: "text"}, Nullable: true}},
			...
			mock: func(m mock) {
				m.ExpectQuery(sqlx.Escape("SELECT * FROM pg_type t JOIN pg_namespace n on t.typnamespace = n.oid WHERE t.typname = $1 AND t.typtype = 'e' AND n.nspname = $2")).
					WithArgs("direction", "public").
					WillReturnRows(sqlmock.NewRows([]string{"name"}))
				m.ExpectQuery(sqlx.Escape("SELECT * FROM pg_type t JOIN pg_namespace n on t.typnamespace = n.oid WHERE t.typname = $1 AND t.typtype = 'e'")).
					WithArgs("state").
			...
			wantPlan: &schema.Plan{
				...
				Changes: []*schema.Change{
					...
					{Cmd: `CREATE TABLE "posts" ("id" integer NOT NULL GENERATED BY DEFAULT AS IDENTITY, "text" text NULL, "directions" "public"."direction"[] NOT NULL, "states" "state"[] NOT NULL) PARTITION BY HASH ("text")`, Reverse: `DROP TABLE "posts"`},
				},
			},
		},
		{
			...
			wantPlan: &schema.Plan{
				Reversible:    true,
				Transactional: true,
				Changes:       []*schema.Change{{Cmd: `CREATE TABLE "posts" ("id" integer NOT NULL GENERATED BY DEFAULT AS IDENTITY (START WITH 1024))`, Reverse: `DROP TABLE "posts"`}},
			},
		},
		{
			changes: []schema.SchemaChange{
		...
		},
		{
			...
			wantPlan: &schema.Plan{
				Reversible:    true,
				Transactional: true,
				Changes:       []*schema.Change{{Cmd: `CREATE TABLE "posts" ("id" integer NOT NULL GENERATED BY DEFAULT AS IDENTITY (INCREMENT BY 2))`, Reverse: `DROP TABLE "posts"`}},
			},
		},
		{
			changes: []schema.SchemaChange{
			...
			wantPlan: &schema.Plan{
				Reversible:    true,
				Transactional: true,
				Changes:       []*schema.Change{{Cmd: `CREATE TABLE "posts" ("id" integer NOT NULL GENERATED BY DEFAULT AS IDENTITY (START WITH 100 INCREMENT BY 2))`, Reverse: `DROP TABLE "posts"`}},
			},
		},
		{
			changes: []schema.SchemaChange{
		...
		},
		{
			...
			wantPlan: &schema.Plan{
				...
				Changes: []*schema.Change{
					{
						Cmd:     `ALTER TABLE "users" ADD COLUMN "name" character varying(255) NOT NULL DEFAULT 'logged_in', ADD COLUMN "last" character varying(255) NOT NULL DEFAULT 'logged_in'`,
						Reverse: `ALTER TABLE "users" DROP COLUMN "last", DROP COLUMN "name"`,
					},
					{
			...
			changes: []schema.SchemaChange{
				&schema.AddTable{
					T: &schema.Table{
						Name: "posts",
						Columns: []*schema.Column{
							{Name: "id", Type: &schema.ColumnType{Type: &SerialType{T: "serial"}, Nullable: true}},
						},
					},
				},
			},
		...
		// Empty qualifier.
		{
			...
			options: []schema.PlanOption{
				func(o *schema.PlanOptions) { o.SchemaQualifier = new(string) },
			},
			mock: func(m mock) {
				m.ExpectQuery(sqlx.Escape("SELECT * FROM pg_type t JOIN pg_namespace n on t.typnamespace = n.oid WHERE t.typname = $1 AND t.typtype = 'e'")).
					WithArgs("enum").
