...
type changesetIoWriter struct {
	...
	// the io.Writer is multipurpose:
	//  - for capturing and transmitting the changeset in long running network migrations
	//  - updating table statistics required for query cost
	// writer io.Writer
...
type ChangesetEntry struct {
	...
	// userSchema: set to true if the schema is user-defined
	UserSchema bool
...
func (c *ChangesetEntry) applyUpdates(ctx context.Context, tx sql.DB, rel *Relation) error {
	if len(c.OldTuple) != len(c.NewTuple) {
		return fmt.Errorf("old and new tuples have different lengths")
	}

	updateSql := fmt.Sprintf("UPDATE %s.%s SET ", rel.Schema, rel.Table)
	...
	for i, v := range oldVals {
		...
		if v == nil {
			whereClause += fmt.Sprintf("%s IS NULL", rel.Columns[i].Name)
		} else {
			whereClause += fmt.Sprintf("%s = $%d", rel.Columns[i].Name, cnt+len(newVals))
			oldArgs = append(oldArgs, v)
			cnt++
		}
	}

	_, err = tx.Execute(ctx, updateSql+" WHERE "+whereClause, append(newVals, oldArgs...)...)
	if err != nil {
		return err
	}
...
func (ce *ChangesetEntry) applyDeletes(ctx context.Context, tx sql.DB, rel *Relation) error {
	deleteSql := fmt.Sprintf("DELETE FROM %s.%s WHERE ", rel.Schema, rel.Table)
	...
	for i, v := range record {
		...
		if v == nil {
			...
			whereClause += fmt.Sprintf("%s = $%d", rel.Columns[i].Name, cnt)
			args = append(args, v)
			cnt++
		}
	}

	if _, err = tx.Execute(ctx, deleteSql+whereClause, args...); err != nil {
		return err
	}
...
func (c *changesetIoWriter) decodeInsert(insert *pglogrepl.InsertMessageV2, relation *pglogrepl.RelationMessageV2) error {
	...
	userSchema := false
	if strings.HasPrefix(relation.Namespace, "ds_") {
		userSchema = true
	}
