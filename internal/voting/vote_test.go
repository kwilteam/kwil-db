...
func Test_Votes(t *testing.T) {
	...
	testCases := []testCase{
		{
			...
			fn: func(t *testing.T, v *voting.VoteProcessor, ds *voting.Datastores, db sql.DB) {
				...
				for i := 0; i < 4; i++ {
					...
					err := v.UpdateVoter(ctx, db, []byte(voterid), 1)
					require.NoError(t, err)
				}

				thresholdPower, err := v.RequiredPower(ctx, db, 2, 3)
				require.NoError(t, err)
		...
		},
		{
			...
			fn: func(t *testing.T, v *voting.VoteProcessor, ds *voting.Datastores, db sql.DB) {
				...
				err = v.CreateResolution(ctx, db, event, 10000, []byte("voter1"))
				require.NoError(t, err)
		...
		},
		{
			...
			fn: func(t *testing.T, v *voting.VoteProcessor, ds *voting.Datastores, db sql.DB) {
				...
				// Update power
				err = v.UpdateVoter(ctx, db, []byte("voter1"), 20)
				require.NoError(t, err)
		...
		},
		{
			...
			fn: func(t *testing.T, v *voting.VoteProcessor, ds *voting.Datastores, db sql.DB) {
				...
				// create vote 1
				err = v.CreateResolution(ctx, db, event1, 2, []byte("voter1"))
				require.NoError(t, err)

				err = v.CreateResolution(ctx, db, event2, 3, []byte("voter1"))
				require.NoError(t, err)

				err = v.CreateResolution(ctx, db, event3, 4, []byte("voter1"))
				require.NoError(t, err)
		...
		},
		{
			...
			fn: func(t *testing.T, v *voting.VoteProcessor, ds *voting.Datastores, db sql.DB) {
				...
				res, err := v.ContainsBody(ctx, db, event.ID())
				require.NoError(t, err)
				require.False(t, res)
		...
		},
		{
			...
			fn: func(t *testing.T, v *voting.VoteProcessor, ds *voting.Datastores, db sql.DB) {
				...
				// add voters
				for i := 0; i < 6; i++ {
					err := v.UpdateVoter(ctx, db, []byte(fmt.Sprintf("voter%d", i)), 10)
					require.NoError(t, err)
				}
				...
				// create vote
				err = v.CreateResolution(ctx, db, event, 10, []byte("voter1"))
				require.NoError(t, err)

				// approve vote
				err = v.Approve(ctx, db, event.ID(), 10, []byte("voter1"))
				require.NoError(t, err)

				err = v.Expire(ctx, db, 20)
				require.NoError(t, err)

				// check that the proposer was not refunded as resolution didnt receive 1/3rd votes.
				acc, err := ds.Accounts.GetAccount(ctx, db, []byte("voter1"))
				require.NoError(t, err)
				require.Equal(t, big.NewInt(0), acc.Balance)
			},
		},
		{
			...
			fn: func(t *testing.T, v *voting.VoteProcessor, ds *voting.Datastores, db sql.DB) {
				...
				err = v.Approve(ctx, db, event.ID(), 10, []byte("voter2"))
				require.NoError(t, err)
				...
				acc, err := ds.Accounts.GetAccount(ctx, db, []byte("voter2"))
				require.NoError(t, err)
		...
		},
		{
			...
			fn: func(t *testing.T, v *voting.VoteProcessor, ds *voting.Datastores, db sql.DB) {
				...
				ids, err := v.ProcessConfirmedResolutions(ctx, db)
				require.NoError(t, err)
				require.Len(t, ids, 1)
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			v, err := voting.NewVoteProcessor(ctx, dbTx, ds.Accounts, ds.Engine, voting.Threshold{Num: 2, Denom: 3}, log.NewStdOut(log.DebugLevel))
			if err != nil {
				t.Fatal(err)
			}
...
func (e *exampleResolutionPayload) Apply(ctx context.Context, db sql.DB, datastores voting.Datastores, proposer []byte, voters []voting.Voter, logger log.Logger) error {
	if e.Account == nil {
