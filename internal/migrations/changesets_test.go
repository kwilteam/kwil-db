...
func TestChangesetMigration(t *testing.T) {
	...
	for idx, cs := range csMigrations {
		...
		total, rcvd, err := getChangesetMetadata(ctx, nestedTx, cs.Height.Int64())
		if idx == 0 {
			require.Error(t, err)
		} else {
			require.NoError(t, err)
		...
		_, err = getChangeset(ctx, nestedTx, height.Int64(), int64(idx))
		require.NoError(t, err)
		require.NoError(t, nestedTx.Commit(ctx))
	}

	// Check if all changesets are received
	total, rcvd, err := getChangesetMetadata(ctx, tx, height.Int64())
	require.NoError(t, err)
	assert.Equal(t, total, rcvd, "total chunks should be equal")

	// Apply the changesets
	err = applyChangeset(ctx, tx, height.Int64(), int64(totalChunks))
	// tx.Commit(ctx)
