...
type GetTableFunc = func(namespace string, tableName string) (table *engine2.Table, found bool)
...
func (s *scopeContext) buildUpsert(node *parse.OnConflict, table *engine2.Table, insertFrom Plan) (ConflictResolution, error) {
	...
	case 1:
		...
		if table.HasPrimaryKey(col.Name) {
			...
			for _, idx := range table.Indexes {
				if (idx.Type == engine2.UNIQUE_BTREE || idx.Type == engine2.PRIMARY) && len(idx.Columns) == 1 && idx.Columns[0] == col.Name {
			...
			if !done {
				...
				for _, con := range table.Constraints {
					if con.Type == engine2.ConstraintUnique && len(con.Columns) == 1 && con.Columns[0] == col.Name {
						arbiterIndex = &IndexColumnConstraint{
							Table:          table.Name,
							Column:         col.Name,
	...
	default:
		...
		for _, idx := range table.Indexes {
			if idx.Type != engine2.UNIQUE_BTREE && idx.Type != engine2.PRIMARY {
				continue
			}
		...
		if !found {
			...
			for name, con := range table.Constraints {
				if con.Type != engine2.ConstraintUnique {
					continue
				}
...
// were passed in.
func checkNullableColumns(tbl *engine2.Table, cols []string) ([]*types.DataType, error) {
