	...
	return stmtFunc(func(exec *executionContext, fn func([]Value) error) error {
		...
		err = funcDef.Func(exec, vals, func(received []Value) error {
			...
			for j, r := range receivers {
				err = exec.allocateVariable(r, received[j])
				if err != nil {
					return err
				}
			}
...
func (i *interpreterPlanner) VisitUseExtensionStatement(p0 *parse.UseExtensionStatement) any {
	...
	return stmtFunc(func(exec *executionContext, fn func([]Value) error) error {
		...
		var ext *PrecompileExtension[any]

		extNamespace, err := initializeExtension(exec, ext, p0.Alias, config)
		if err != nil {
			return err
		}

		err = executeOnUse(exec, extNamespace)
		if err != nil {
			return err
		}
