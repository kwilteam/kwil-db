...
func (a *Action) FromAST(ast *parse.CreateActionStatement) error {
	...
	for _, m := range ast.Modifiers {
		...
		if mod == ModifierPublic || mod == ModifierPrivate || mod == ModifierSystem {
...
const (
	// ModifierPublic means that the action is public.
	ModifierPublic Modifier = "PUBLIC"
	// ModifierPrivate means that the action is private.
	ModifierPrivate Modifier = "PRIVATE"
	// ModifierSystem means that the action can only be called by the system.
...
func stringToMod(s string) (Modifier, error) {
	switch strings.ToLower(s) {
	case "public":
		return ModifierPublic, nil
	case "private":
		return ModifierPrivate, nil
	case "system":
		return ModifierSystem, nil
	case "owner":
		return ModifierOwner, nil
	case "view":
		return ModifierView, nil
	default:
