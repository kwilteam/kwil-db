...
import (
	...
	"github.com/kwilteam/kwil-db/internal/engine2/planner/logical"
)
...
func (e *executionContext) getTable(namespace, tableName string) (*engine2.Table, bool) {
...
func (e *executionContext) reloadTables() error {
	...
	ns.tables = make(map[string]*engine2.Table)
...
func (e *executionContext) canExecute(namespace string, name string, modifiers Modifiers) error {
	// if the ctx cannot mutate state and the action is not a view (and thus might try to mutate state),
	// then return an error
	if !modifiers.Has(ModifierView) && !e.canMutateState {
	...
	if modifiers.Has(ModifierPrivate) && e.scope.namespace != namespace {
	...
	if modifiers.Has(ModifierSystem) && e.scope.parent == nil {
	...
	if modifiers.Has(ModifierOwner) && !e.interpreter.accessController.IsOwner(e.txCtx.Caller) {
...
func (e *executionContext) app() *common.App {
	//TODO: we need to set the app.Engine
