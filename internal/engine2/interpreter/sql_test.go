	...
	tests := []testcase{
		...
		},
		{
			...
			fn: func(ctx context.Context, db sql.DB) {
				...
				wantSchemas := map[string]map[string]*engine2.Table{
					"main": {
						"users": {
							Name: "users",
							Columns: []*engine2.Column{
								{
									Name:         "id",
							...
							Indexes: []*engine2.Index{
								...
								},
								{
									Name:    "users_name_idx",
									Columns: []string{"name"},
									Type:    engine2.UNIQUE_BTREE,
								},
								{
									Name:    "users_pkey",
									Columns: []string{"id"},
									Type:    engine2.PRIMARY,
								},
								{
							...
							Constraints: map[string]*engine2.Constraint{
								"users_name_check": {
									Type:    engine2.ConstraintCheck,
									Columns: []string{"name"},
								},
								"users_age_check": {
									Type:    engine2.ConstraintCheck,
				...
				tables := map[string]map[string]*engine2.Table{}

				for schemaName := range wantSchemas {
					tbls, err := listTablesInNamespace(ctx, db, schemaName)
					require.NoError(t, err)
					tables[schemaName] = map[string]*engine2.Table{}
