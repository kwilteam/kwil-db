...
import (
	...
	pggenerate "github.com/kwilteam/kwil-db/internal/engine2/pg_generate"
...
func Test_PgGenerate(t *testing.T) {
	...
	tests := []testcase{
		...
		},
		{
			...
			want: "SELECT * FROM kwil.tbl WHERE col = current_setting('ctx.caller');",
		},
		{
		...
		},
		{
			...
			want:   "SELECT * FROM kwil.tbl WHERE col = $1;",
			params: []string{"$param"},
		},
		{
		...
		},
		{
			name: "No parameters",
			sql:  "SELECT * FROM tbl;",
			want: "SELECT * FROM kwil.tbl;",
		},
		{
			name:   "Parameter in function call",
			sql:    "SELECT * FROM tbl WHERE col = abs($pwd);",
			want:   "SELECT * FROM kwil.tbl WHERE col = abs($1);",
			params: []string{"$pwd"},
		},
		{
			name:   "Parameter in JOIN condition",
			sql:    "SELECT t1.col, t2.col FROM t1 JOIN t2 ON t1.id = t2.id AND t1.name = $name;",
			want:   "SELECT t1.col, t2.col FROM kwil.t1 INNER JOIN kwil.t2 ON t1.id = t2.id AND t1.name = $1;",
			params: []string{"$name"},
		},
		{
			name: "window function",
			sql:  "SELECT col1, col2, SUM(col3) OVER (PARTITION BY col1 ORDER BY col2) FROM tbl;",
			want: "SELECT col1, col2, sum(col3) OVER (PARTITION BY col1 ORDER BY col2 ASC NULLS LAST) FROM kwil.tbl;",
		},
		{
			name: "array access",
			sql:  "SELECT col1[1], col2[2] FROM tbl;",
			want: "SELECT col1[1], col2[2] FROM kwil.tbl;",
		},
		{
			name: "array slice",
			sql:  "SELECT col1[1:2], col2[2:], col3[:3] FROM tbl;",
			want: "SELECT col1[1:2], col2[2:], col3[:3] FROM kwil.tbl;",
		},
		{
			name: "make array",
			sql:  "SELECT ARRAY[col1, col2] FROM tbl;",
			want: "SELECT ARRAY[col1, col2] FROM kwil.tbl;",
		},
		{
			name: "type cast",
			sql:  "SELECT col1::INT8, (col2::TEXT)::INT8 FROM tbl;",
			want: "SELECT col1::INT8, (col2::TEXT)::INT8 FROM kwil.tbl;",
		},
		{
			name: "arithmetics",
			sql:  "SELECT col1 + col2, col1 - col2, col1 * col2, col1 / col2 FROM tbl;",
			want: "SELECT col1 + col2, col1 - col2, col1 * col2, col1 / col2 FROM kwil.tbl;",
		},
		{
			name: "comparison",
			sql:  "SELECT col1 = col2, col1 <> col2, col1 < col2, col1 <= col2, col1 > col2, col1 >= col2 FROM tbl;",
			want: "SELECT col1 = col2, col1 <> col2, col1 < col2, col1 <= col2, col1 > col2, col1 >= col2 FROM kwil.tbl;",
		},
		{
			name: "unary",
			sql:  "SELECT +col1, -col2 FROM tbl;",
			want: "SELECT +col1, -col2 FROM kwil.tbl;",
		},
		{
			name: "logical",
			sql:  "SELECT col1 AND col2, col1 OR col2, NOT col1 FROM tbl;",
			want: "SELECT col1 AND col2, col1 OR col2, NOT col1 FROM kwil.tbl;",
		},
		{
			name: "case",
			sql:  "SELECT CASE WHEN col1 = 1 THEN 'one' ELSE 'other' END FROM tbl;",
			want: "SELECT CASE WHEN col1 = 1 THEN 'one' ELSE 'other' END FROM kwil.tbl;",
		},
		{
			name: "collate",
			sql:  "SELECT col1 from tbl where name = 'foo' collate nocase;",
			want: "SELECT col1 FROM kwil.tbl WHERE name = 'foo' COLLATE nocase;",
		},
		{
			name: "is null",
			sql:  "SELECT col1 IS NULL, col2 IS NOT NULL FROM tbl;",
			want: "SELECT col1 IS NULL, col2 IS NOT NULL FROM kwil.tbl;",
		},
		{
			name: "between",
			sql:  "SELECT col1 BETWEEN 1 AND 10 FROM tbl;",
			want: "SELECT col1 BETWEEN 1 AND 10 FROM kwil.tbl;",
		},
		{
			name: "in",
			sql:  "SELECT col1 IN (1, 2, 3) FROM tbl;",
			want: "SELECT col1 IN (1, 2, 3) FROM kwil.tbl;",
		},
		{
			name: "like",
			sql:  "SELECT col1 LIKE 'foo%' FROM tbl WHERE col2 NOT LIKE '%bar' AND col3 ILIKE 'baz%';",
			want: "SELECT col1 LIKE 'foo%' FROM kwil.tbl WHERE col2 NOT LIKE '%bar' AND col3 ILIKE 'baz%';",
		},
		{
			name: "exists",
			sql:  "SELECT EXISTS (SELECT 1 FROM tbl WHERE col1 = 1);",
			want: "SELECT EXISTS (SELECT 1 FROM kwil.tbl WHERE col1 = 1);",
		},
		{
			name: "subquery",
			sql:  "SELECT (SELECT col1 FROM tbl WHERE col2 = 1) FROM tbl2;",
			want: "SELECT (SELECT col1 FROM kwil.tbl WHERE col2 = 1) FROM kwil.tbl2;",
		},
		{
			name: "common table expression",
			sql:  "WITH cte AS (SELECT * FROM tbl) SELECT * FROM cte;",
			want: "WITH cte AS (SELECT * FROM kwil.tbl) SELECT * FROM cte;",
		},
		{
			name: "recursive common table expression",
			sql:  "WITH RECURSIVE cte AS (SELECT * FROM tbl) SELECT * FROM cte;",
			want: "WITH RECURSIVE cte AS (SELECT * FROM kwil.tbl) SELECT * FROM cte;",
		},
		// ddl
		{
			name: "Create table",
