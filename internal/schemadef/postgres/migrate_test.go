...
func TestPlanChanges(t *testing.T) {
	tests := []struct {
		...
		{
			changes: func() []schema.SchemaChange {
				s := schema.New("s1")
				t := schema.NewTable("t1").SetSchema(s)
				...
				return []schema.SchemaChange{change}
			}(),
			wantPlan: &schema.Plan{
				Reversible:    true,
				Transactional: true,
	...
	for i, tt := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			drv, err := Open(db)
			require.NoError(t, err)
			plan, err := drv.PlanChanges(context.Background(), tt.changes, tt.options...)
			if tt.wantErr {
