...
func (r *TypeRegistry) PrintType(typ *Type) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("spec: type %q not found in registry", typ.T)
	}
...
func (r *TypeRegistry) Register(specs ...*TypeSpec) error {
	for _, s := range specs {
		if err := validSpec(s); err != nil {
			return fmt.Errorf("spec: invalid typespec %q: %w", s.Name, err)
		}
		if _, exists := r.findT(s.T); exists {
			return fmt.Errorf("spec: type with T of %q already registered", s.T)
		}
		if _, exists := r.findName(s.Name); exists {
			return fmt.Errorf("spec: type with name of %q already registered", s.T)
		}
		r.r = append(r.r, s)
	}
	return nil
		...
		case reflect.Slice:
			...
			for i := 0; i < field.Len(); i++ {
				...
				if fi.Kind() != reflect.String {
					return nil, errors.New("spec: only string slices currently supported")
				}
				lits = append(lits, strconv.Quote(fi.String()))
			}
			s.Attrs = append([]*Attr{ListAttr(attr.Name, lits...)}, s.Attrs...)
		default:
			return nil, fmt.Errorf("spec: unsupported attr kind %s for attribute %q of %q", attr.Kind, attr.Name, typeSpec.Name)
		}
	}
	return s, nil
}
...
// TypeSpecOption configures a hcl.TypeSpec.
