...
type (
	// State is used to evaluate and marshal HCL documents and stores a configuration for these operations.
	State struct {
		config *Config
	}
	...
	Marshaler interface {
		// MarshalSpec marshals the provided input into a valid HCL document.
...
// MarshalSpec implements Marshaler for HCL documents.
func (s *State) MarshalSpec(v any) ([]byte, error) {
	r := &Resource{}
	if err := r.Scan(v); err != nil {
		return nil, fmt.Errorf("hcl: failed scanning %T to resource: %w", v, err)
	}
	return s.encode(r)
}
...
func (s *State) Eval(parsed *hclparse.Parser, v any, input map[string]string) error {
	...
	if err := spec.As(v); err != nil {
		return fmt.Errorf("hcl: failed reading spec as %T: %w", v, err)
	}
	return nil
}

// EvalBytes evaluates the data byte-slice as an HCL document using the input variables
...
// patchRefs recursively searches for hcl.Ref under the provided hcl.Resource
...
// resource converts the hcl file to a hcl.Resource.
func (s *State) resource(ctx *hcl.EvalContext, file *hcl.File) (*Resource, error) {
	body, ok := file.Body.(*hclsyntax.Body)
	if !ok {
		return nil, fmt.Errorf("hcl: expected remainder to be of type *hclsyntax.Body")
	}
...
func extractLiteralValue(value cty.Value) (*LiteralValue, error) {
	...
	default:
		return nil, fmt.Errorf("hcl: unsupported type %q", value.Type().GoString())
	}
}
...
// encode encodes the give *hcl.Resource into a byte slice containing an HCL document representing it.
