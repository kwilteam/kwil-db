...
func StoreRole(role *dba.Role, d KVBasic) error {
...
func getRoleDDLKey(role *dba.Role) []byte {
	return append([]byte(role.GetName()), []byte("ddl")...)
}

func getRolePQKey(role *dba.Role, pq string) []byte {
	k := append([]byte(role.GetName()), []byte("pq")...)
	return append(k, []byte(pq)...)
}

func (d *DB) GetRole(roleName string) (*dba.Role, error) {
	var role dba.Role
	role.Name = roleName
	...
	role.Permissions = dba.Permissions{
		DDL:                  bytes2Bool(ddl),
