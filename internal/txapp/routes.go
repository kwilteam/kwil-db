...
func init() {
	err := errors.Join(
		...
		RegisterRoute(transactions.PayloadTypeVoteResolution, NewRoute(&voteResolutionRoute{})),
	)
	if err != nil {
...
func (d *createResolutionRoute) PreTx(ctx common.TxContext, svc *common.Service, tx *transactions.Transaction) (transactions.TxCode, error) {
	if ctx.BlockContext.ChainContext.NetworkParameters.InMigration {
		return transactions.CodeNetworkInMigration, fmt.Errorf("cannot create resolution during migration")
	}
...
type voteResolutionRoute struct {
	resolutionID   *types.UUID
	resolutionType string
}

var _ consensus.Route = (*voteResolutionRoute)(nil)

func (d *voteResolutionRoute) Name() string {
	return transactions.PayloadTypeVoteResolution.String()
}

func (d *voteResolutionRoute) Price(ctx context.Context, app *common.App, tx *transactions.Transaction) (*big.Int, error) {
	return ValidatorVoteIDPrice, nil
}

func (d *voteResolutionRoute) PreTx(ctx common.TxContext, svc *common.Service, tx *transactions.Transaction) (transactions.TxCode, error) {
	if ctx.BlockContext.ChainContext.NetworkParameters.InMigration {
		return transactions.CodeNetworkInMigration, fmt.Errorf("cannot vote during migration")
	}

	vote := &transactions.VoteResolution{}
	err := vote.UnmarshalBinary(tx.Body.Payload)
	if err != nil {
		return transactions.CodeEncodingError, err
	}
	...
	// Check if its a valid event type
	_, err = resolutions.GetResolution(vote.ResolutionType)
	if err != nil {
		return transactions.CodeInvalidResolutionType, err
	}

	d.resolutionType = vote.ResolutionType
	d.resolutionID = vote.ResolutionID
	return 0, nil
}

func (d *voteResolutionRoute) InTx(ctx common.TxContext, app *common.App, tx *transactions.Transaction) (transactions.TxCode, error) {
	// ensure the sender is a validator
	power, err := getVoterPower(ctx.Ctx, app.DB, tx.Sender)
	if err != nil {
		return transactions.CodeUnknownError, err
