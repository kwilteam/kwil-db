...
func formatForeignProcReturnsTable(procedureName string, inputs []*types.NamedType, returns []*types.NamedType) (string, error) {
	// We need to format the return types and names.
	// We can do this by iterating over the return types and names,
	// and formatting them as a string. We can then join them with a comma.

	ins, err := formatNamedTypes(inputs, true)
	if err != nil {
		return "", err
	}

	outs, err := formatNamedTypes(returns, false)
	if err != nil {
		return "", err
	}
	...
	str.WriteString(`CREATE OR REPLACE FUNCTION _fp_%s(_dbid TEXT, _name TEXT%s)
	RETURNS TABLE(%s)
	AS $$
	BEGIN
	...
	str.WriteString(`)', _dbid, _name%s);
	END;
	$$ LANGUAGE plpgsql;`)

	return fmt.Sprintf(str.String(), procedureName, ins, outs, getNames(inputs)), nil
}

// getNames the names delimited by commas.
// It will add a comma to the beginning, since this should always
// be used after the procedure name.
func getNames(types []*types.NamedType) string {
	str := strings.Builder{}
	for _, t := range types {
		str.WriteString(", ")
...
// formatNamedTypes formats named types as identifiers and types, delimited by a comma.
// It can optionally be set to add a comma before the first element.
func formatNamedTypes(types []*types.NamedType, prefixComma bool) (string, error) {
	str := strings.Builder{}
	for i, t := range types {
		if i > 0 || prefixComma {
			str.WriteString(", ")
