...
// It stores a flattened and reversed list of expressions, which can be used to check
...
// It will flatten and reverse the expressions, so that they can be used to check
...
func newAggregateChecker(exprs []LogicalExpr) (*aggregateChecker, error) {
	...
	for _, expr := range exprs {
		...
		// reverse the list for efficient post-order traversal
		// in the check method
		slices.Reverse(plan)
...
func (a *aggregateChecker) check(e LogicalExpr) error {
	...
	// and if a column is found that is not aggregated, we will reverse the expression list
	// and check that resulting list contains one of the values in the included list.
	...
	// now we can reverse the list, and for each column, check that it is in the included list
