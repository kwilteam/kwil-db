...
import (
	...
	"github.com/kwilteam/kwil-db/internal/engine/procedural/parser"
	"github.com/kwilteam/kwil-db/internal/engine/procedural/visitors/traverse"
...
func CleanProcedure(stmts []parser.Statement, proc *types.Procedure, currentSchema *types.Schema, contextPrefix string, knownVars map[string]*types.DataType) (params []*types.NamedType, err error) {
	defer func() {
		if e := recover(); e != nil {
			err = e.(error)
		}
...
func (c *cleaner) cleanSQL(ast tree.AstNode) {
	err := sqlanalyzer.CleanAST(ast, c.currentSchema.Tables, c.currentSchema.DBID())
	if err != nil {
		panic(err)
	}
