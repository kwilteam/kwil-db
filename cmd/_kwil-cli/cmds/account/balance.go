...
func balanceCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) > 0 {
				clientFlags = helpers.WithoutPrivateKey
			...
			return helpers.DialClient(cmd.Context(), cmd, clientFlags, func(ctx context.Context, cl clientType.Client, conf *config.KwilCliConfig) error {
				if len(acctID) == 0 {
