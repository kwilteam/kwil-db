...
type Committable interface {
	...
	EndCommit(ctx context.Context, appender func([]byte) error) (commitId []byte, err error)
...
type Wal interface {
	...
	// the io.Reader will return an io.EOF when it has reached the end of the record
