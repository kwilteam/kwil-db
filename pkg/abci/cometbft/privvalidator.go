...
import (
	...
	numBytes "github.com/kwilteam/kwil-db/pkg/utils/numbers/bytes"
)
...
func NewValidatorSigner(ed25519Key []byte, kvStore AtomicKV) (*ValidatorSigner, error) {
...
// It is part of the cometTypes.PrivValidator interface
func (v *ValidatorSigner) SignProposal(chainID string, proposal *cmtproto.Proposal) error {
	...
	if sameHRS {
		if bytes.Equal(signBytes, v.lastSignedState.SignBytes) {
			...
			return fmt.Errorf("proposal sign bytes differ from last sign bytes")
		}
	}
...
// It is part of the cometTypes.PrivValidator interface
func (v *ValidatorSigner) SignVote(chainID string, vote *cmtproto.Vote) error {
...
type lastSignState struct {
	...
	SignBytes cmtbytes.HexBytes

	// kv is the keyvalue store that this lastSignState is persisted to
	kv AtomicKV
}
...
func (l *lastSignState) store() error {
	...
	err = errors.Join(
		tx.Set(heightKey, numBytes.Int64ToBytes(l.Height)),
		tx.Set(roundKey, numBytes.Int32ToBytes(l.Round)),
		tx.Set(stepKey, []byte{byte(l.Step)}),
		tx.Set(signatureKey, l.Signature),
		tx.Set(signBytesKey, l.SignBytes.Bytes()),
	)
	if err != nil {
		return err
	}
...
func (l *lastSignState) loadLatest() (err error) {
	// if we encounter a key not found error, we set all fields to zero values
	// if not, then there is a KV store issue, and we do not want to sign for risk
	// of double signing
	defer func() {
	...
	height, err := tx.Get(heightKey)
	if err != nil {
		return err
	}

	round, err := tx.Get(roundKey)
	if err != nil {
		return err
	}

	step, err := tx.Get(stepKey)
	if err != nil {
		return err
	}
	// guard against index out of range panic
	if len(step) != 1 {
		return fmt.Errorf("invalid step length.  received: %d, expected: 1", len(step))
	}

	signature, err := tx.Get(signatureKey)
	if err != nil {
		return err
	}

	signBytes, err := tx.Get(signBytesKey)
	if err != nil {
		return err
	}

	l.Height = numBytes.BytesToInt64(height)
	l.Round = numBytes.BytesToInt32(round)
	l.Step = int8(step[0])
	l.Signature = signature
	l.SignBytes = cmtbytes.HexBytes(signBytes)
...
// if they do not, it returns an error
...
// AtomicKV is an interface for a keyvalue store.
// This should be durable, and fsync (or equivalent) after every write/commit
type AtomicKV interface {
	// BeginTransaction starts a new transaction
	BeginTransaction() (kv.Transaction, error)
}

var (
	heightKey    = []byte{byte(10)}
	roundKey     = []byte{byte(20)}
	stepKey      = []byte{byte(30)}
	signatureKey = []byte{byte(40)}
	signBytesKey = []byte{byte(50)}
)
