...
func TestParseRawSQL_allowed(t *testing.T) {
	tests := []struct {
		name  string
		input string
	}{
		...
		{"insert with values upsert without target", "insert into t1 (a,b) values (1,2) on conflict do nothing"},
		{"insert with values upsert with target", "insert into t1 (a,b) values (1,2) on conflict (c1 collate collate_name asc) do nothing"},
		{"insert with values upsert with target and where", "insert into t1 (a,b) values (1,2) on conflict(c1) where 1 do nothing"},
		{"insert with values upsert with update", "insert into t1 (a,b) values (1,2) on conflict do update set b=excluded.b"},
		{"insert with values upsert with update and where", "insert into t1 (a,b) values (1,2) on conflict do update set b=excluded.b where 1"},
