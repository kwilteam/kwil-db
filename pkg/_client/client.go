...
func New(ctx context.Context, rpcUrl string, opts ...ClientOption) (*KwilClient, error) {
	...
	cfg, err := c.grpc.GetConfig(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to get service config from kwil provider: %w", err)
	}

	c.ChainCode = chainTypes.ChainCode(cfg.ChainCode)
	c.ProviderAddress = cfg.ProviderAddress
	c.EscrowContractAddress = cfg.PoolAddress
...
func (c *KwilClient) GetConfig(ctx context.Context) (*grpc.SvcConfig, error) {
	return c.grpc.GetConfig(ctx)
}
