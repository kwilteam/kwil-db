...
func (v *Validator) validateQuery(q *databases2.SQLQuery[anytype.KwilAny]) error {
	if !q.Type.IsValid() {
		return fmt.Errorf(`invalid query type`)
	}

	// check if query name is valid
	err := CheckName(q.Name, databases2.MAX_QUERY_NAME_LENGTH)
	if err != nil {
...
func validateQueryType(q *databases2.SQLQuery[anytype.KwilAny]) error {
	// check if query has too many params or where clauses
	if len(q.Params) > execution2.MAX_PARAM_PER_QUERY || len(q.Where) > execution2.MAX_WHERE_PER_QUERY {
		return fmt.Errorf(`query "%s" cannot have more than %d parameters or %d where clauses`, q.Name, execution2.MAX_PARAM_PER_QUERY, execution2.MAX_WHERE_PER_QUERY)
	}

	// check if insert has where clause
	if q.Type == execution2.INSERT && len(q.Where) > 0 {
	...
	if q.Type == execution2.INSERT || q.Type == execution2.UPDATE {
		if len(q.Params) == 0 {
	...
	if q.Type == execution2.UPDATE || q.Type == execution2.DELETE {
		if len(q.Where) == 0 {
	...
	if q.Type == execution2.DELETE && len(q.Params) > 0 {
...
func validateInput(input databases2.Input[anytype.KwilAny], table *databases2.Table[anytype.KwilAny]) error {
	...
	if input.GetStatic() {
		...
		if input.GetValue().Value() == nil && input.GetModifier() != execution2.CALLER {
		...
		if col.Type != input.GetValue().Type() && input.GetModifier() != execution2.CALLER {
		...
		if input.GetModifier() == execution2.CALLER {
