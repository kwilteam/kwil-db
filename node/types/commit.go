...
const (
	// Disagree status indicates that the validator disagrees with the proposed block and
	// responded with a NACK.
	AckStatusDisagree AckStatus = iota
	// Agree status indicates that the validator agrees with the proposed block and
	// computed the same AppHash after applying the block.
	AckStatusAgree
	// Diverge status indicates that the validator agrees with the proposed block but
	// the application state diverged after applying the block leading to a different AppHash.
	AckStatusDiverge
)
...
// VoteInfo contains the details of all the votes from validators that contributed
// to the block's commit decision. This information is generated by the leader
// before committing the block and is broadcasted to the network, allowing validators
// to verify the commit decision before accepting the block.
...
// TODO: should this sign the nack status as well? and the proof?
