...
type colOutputter struct {
	catalog *catalog.Catalog
}

// OutputColumns determines which columns a statement will output
func (c *colOutputter) OutputColumns(stmt ast.Node) ([]*catalog.Column, error) {
	...
	for _, col := range cols {
		catCols = append(catCols, &catalog.Column{
			Name:      col.Name,
			Type:      &catalog.QualName{Name: col.DataType},
...
func outputColumns(qc *QueryCatalog, node ast.Node) ([]*Column, error) {
	...
	for _, target := range targets.Items {
		...
		case *ast.A_Expr:
			...
			switch {
			case isComparisonOperator(astutils.Join(n.Name, "")):
				// TODO: Generate a name for these operations
				cols = append(cols, &Column{Name: name, DataType: "bool", NotNull: true})
			case isMathematicalOperator(astutils.Join(n.Name, "")):
...
func sourceTables(qc *QueryCatalog, node ast.Node) ([]*Table, error) {
	...
	for _, item := range list.Items {
		...
		case *ast.RangeSubselect:
			...
			tables = append(tables, &Table{
				Rel:     &catalog.QualName{Name: *n.Alias.Aliasname},
				Columns: cols,
			})
