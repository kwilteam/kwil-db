...
func (c *Catalog) getTable(tableName *ast.TableName) (*Schema, *Table, error) {
	...
	table, _, err := schema.GetTable(tableName.Name)
	if err != nil {
		return nil, nil, err
	}
...
func (c *Catalog) alterTableAST(stmt *ast.AlterTableStmt) error {
...
func (c *Catalog) alterTableSetSchemaAST(stmt *ast.AlterTableSetSchemaStmt) error {
	...
	tbl, idx, err := oldSchema.GetTable(stmt.Table.Name)
	if err != nil {
		return err
	}
	...
	if _, _, err := newSchema.GetTable(stmt.Table.Name); err == nil {
...
func (c *Catalog) createTableAST(stmt *ast.CreateTableStmt) error {
	ns := stmt.Name.Schema
	if ns == "" {
		ns = c.DefaultSchema
	}
	...
	_, _, err = schema.GetTable(stmt.Name.Name)
	...
	for _, inheritTable := range stmt.Inherits {
		t, _, err := schema.GetTable(inheritTable.Name)
		if err != nil {
			return err
		}
	...
	if stmt.ReferTable != nil {
		...
		for _, col := range stmt.Cols {
			...
			if col.Vals != nil {
				...
				if err := c.createEnumAST(s); err != nil {
					return err
				}
				tc.Type = typeName
			}
...
func (c *Catalog) dropTableAST(stmt *ast.DropTableStmt) error {
	for _, name := range stmt.Tables {
		...
		schema, err := c.GetSchema(ns)
		if errors.Is(err, sqlerr.ErrNotFound) && stmt.IfExists {
			continue
		} else if err != nil {
			return err
		}

		_, idx, err := schema.GetTable(name.Name)
		if errors.Is(err, sqlerr.ErrNotFound) && stmt.IfExists {
			continue
		} else if err != nil {
			return err
...
func (c *Catalog) renameColumnAST(stmt *ast.RenameColumnStmt) error {
...
func (c *Catalog) renameTableAST(stmt *ast.RenameTableStmt) error {
	sch, tbl, err := c.getTable(stmt.Table)
	if err != nil {
		return err
	}
	if _, _, err := sch.GetTable(*stmt.NewName); err == nil {
...
func (c *Catalog) createTableAsAST(stmt *ast.CreateTableAsStmt, colGen ColumnGenerator) error {
	...
	_, _, err = schema.GetTable(tbl.Rel.Name)
	if err == nil {
