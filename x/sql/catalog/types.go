...
func (c *Catalog) createEnumAST(stmt *ast.CreateEnumStmt) error {
	...
	if _, _, err := schema.GetTable(tbl.Name); err == nil {
...
func (c *Catalog) createCompositeTypeAST(stmt *ast.CompositeTypeStmt) error {
	ns := stmt.TypeName.Schema
	if ns == "" {
		ns = c.DefaultSchema
	}
...
func (c *Catalog) alterTypeRenameValueAST(stmt *ast.AlterTypeRenameValueStmt) error {
	ns := stmt.Type.Schema
	if ns == "" {
		ns = c.DefaultSchema
	}
...
func (c *Catalog) alterTypeAddValueAST(stmt *ast.AlterTypeAddValueStmt) error {
	ns := stmt.Type.Schema
	if ns == "" {
		ns = c.DefaultSchema
	}
...
func (c *Catalog) dropTypeAST(stmt *ast.DropTypeStmt) error {
...
func (c *Catalog) renameTypeAST(stmt *ast.RenameTypeStmt) error {
	if stmt.NewName == nil {
	...
	if _, _, err := schema.GetTable(newName); err == nil {
