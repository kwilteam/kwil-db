...
func (_ *task_error[T]) OnCompleteRun(fn Runnable)               { fn() }
func (r *task_error[T]) OnComplete(fn Handler[T])                { fn(x.AsDefault[T](), r.err) }
func (r *task_error[T]) ThenAsync(_ ValueHandler[T]) Task[T]     { return r }
func (r *task_error[T]) CatchAsync(fn ErrorHandler) Task[T]      { return r._catchAsync(fn) }
func (r *task_error[T]) WhenCompleteAsync(fn Handler[T]) Task[T] { return r._whenCompleteAsync(fn) }
func (r *task_error[T]) OnCompleteRunAsync(fn Runnable)          { r._onCompleteRunAsync(fn) }
func (r *task_error[T]) OnCompleteAsync(fn Handler[T])           { r._onCompleteAsync(fn) }
...
func (r *task_error[T]) _catchAsync(fn ErrorHandler) Task[T] {
	go func(r *task_error[T], fn ErrorHandler) {
		fn(r.err)
	}(r, fn)
	return r
}

func (r *task_error[T]) _whenCompleteAsync(fn Handler[T]) Task[T] {
	go func(r *task_error[T], fn Handler[T]) {
		fn(x.AsDefault[T](), r.err)
	}(r, fn)
	return r
}

func (r *task_error[T]) _onCompleteRunAsync(fn Runnable) {
	go func(r *task_error[T], fn Runnable) {
		fn()
	}(r, fn)
}

func (r *task_error[T]) _onCompleteAsync(fn Handler[T]) {
	go func(r *task_error[T], fn Handler[T]) {
		fn(x.AsDefault[T](), r.err)
	}(r, fn)
}
