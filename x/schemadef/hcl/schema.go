...
type (
	...
	TypeSpec struct {
		// Name is the identifier for the type in a DDL document.
		Name string
...
func (a *Attr) Refs() ([]*Ref, error) {
	...
	for _, v := range l.V {
		r, ok := v.(*Ref)
		if !ok {
			return nil, fmt.Errorf("hcl: expected %T to be Ref", v)
		}
		refs = append(refs, r)
	}
	return refs, nil
...
func (a *Attr) Strings() ([]string, error) {
	...
	for _, item := range l.V {
		sv, err := StrVal(item)
		if err != nil {
			return nil, fmt.Errorf("hcl: failed parsing item %q to string: %w", item, err)
		}
		out = append(out, sv)
	}
	return out, nil
...
func StrVal(v Value) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("hcl: expected %T to be LiteralValue", v)
	}
...
func BoolVal(v Value) (bool, error) {
	lit, ok := v.(*LiteralValue)
	if !ok {
		return false, fmt.Errorf("hcl: expected %T to be LiteralValue", v)
	}
	b, err := strconv.ParseBool(lit.V)
	if err != nil {
		return false, fmt.Errorf("hcl: failed parsing %q as bool: %w", lit.V, err)
	}
	return b, nil
}
...
// StrLitAttr is a helper method for constructing *hcl.Attr instances that contain literal values
