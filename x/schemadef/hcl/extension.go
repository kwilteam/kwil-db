...
func (r registry) implementers(i reflect.Type) ([]string, error) {
	if i.Kind() != reflect.Interface {
		return nil, fmt.Errorf("hcl: expected interface got %s", i.Kind())
	}
	var names []string
...
func Register(name string, ext any) {
	...
	if ext == nil {
		panic("hcl: Register extension is nil")
	}
	if _, dup := extensions[name]; dup {
		panic("hcl: Register called twice for type " + name)
	}
	extensions[name] = ext
}
...
func (r *Resource) As(target any) error {
	...
	for _, ft := range specFields(target) {
		...
		case ft.isName() && !hasAttr(r, ft.tag):
			if seenName {
				return errors.New("hcl: extension must have only one isName field")
			}
			seenName = true
			if field.Kind() != reflect.String {
				return errors.New("hcl: extension isName field must be of type string")
			}
			field.SetString(r.Name)
		case ft.isQualifier():
			if seenQualifier {
				return errors.New("hcl: extension must have only one qualifier field")
			}
			seenQualifier = true
	...
	for attrName := range existingAttrs {
		...
		if !ok {
			return fmt.Errorf("hcl: expected attr %q to exist", attrName)
		}
		extras.SetAttr(attr)
	}
...
func validateStructPtr(target any) error {
	...
	if typeOf.Kind() != reflect.Ptr {
		return errors.New("hcl: expected target to be a pointer")
	}
	if typeOf.Elem().Kind() != reflect.Struct {
		return errors.New("hcl: expected target to be a pointer to a struct")
	}
	return nil
}
...
func setChildSlice(field reflect.Value, children []*Resource) error {
	if field.Type().Kind() != reflect.Slice {
		return fmt.Errorf("hcl: expected field to be of kind slice")
	}
	if len(children) == 0 {
		return nil
	}
...
func setField(field reflect.Value, attr *Attr) error {
	...
	case reflect.String:
		s, err := attr.String()
		if err != nil {
			return fmt.Errorf("hcl: value of attr %q cannot be read as string: %w", attr.K, err)
		}
		field.SetString(s)
	case reflect.Int, reflect.Int64:
		i, err := attr.Int()
		if err != nil {
			return fmt.Errorf("hcl: value of attr %q cannot be read as integer: %w", attr.K, err)
		}
		field.SetInt(int64(i))
	case reflect.Bool:
		b, err := attr.Bool()
		if err != nil {
			return fmt.Errorf("hcl: value of attr %q cannot be read as bool: %w", attr.K, err)
		}
		field.SetBool(b)
	case reflect.Ptr:
		if err := setPtr(field, attr.V); err != nil {
			return fmt.Errorf("hcl: failed setting pointer field %q: %w", attr.K, err)
		}
	case reflect.Interface:
		field.Set(reflect.ValueOf(attr.V))
	default:
		return fmt.Errorf("hcl: unsupported field kind %q", field.Kind())
	}
	return nil
}
...
func setSliceAttr(field reflect.Value, attr *Attr) error {
	...
	if !ok {
		return fmt.Errorf("hcl: field is of type slice but attr %q does not contain a ListValue", attr.K)
	}
	typ := field.Type().Elem()
...
func (r *Resource) Scan(ext any) error {
	...
	for _, ft := range specFields(ext) {
		...
		case ft.isName():
			if field.Kind() != reflect.String {
				return errors.New("hcl: extension name field must be string")
			}
			r.Name = field.String()
		case ft.isQualifier():
			if field.Kind() != reflect.String {
				return errors.New("hcl: extension qualifer field must be string")
			}
...
func scanPtr(key string, r *Resource, field reflect.Value) error {
	...
	default:
		return fmt.Errorf("hcl: unsupported pointer to %s", e)
	}
	r.SetAttr(attr)
	return nil
}
...
func scanAttr(key string, r *Resource, field reflect.Value) error {
	...
	case reflect.Interface:
		...
		v, ok := i.(Value)
		if !ok {
			return fmt.Errorf("hcl: unsupported interface type %T for field %q", i, key)
		}
		r.SetAttr(&Attr{
			K: key,
			V: v,
...
func scanSliceAttr(key string, r *Resource, field reflect.Value) error {
	...
	case reflect.Ptr:
		if typ.Elem() != reflect.TypeOf(&Ref{}) {
			return fmt.Errorf("hcl: currently on ref slice values supported, got %s", typ)
		}
		for i := 0; i < field.Len(); i++ {
