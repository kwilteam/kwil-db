...
// varDef is an HCL resource that defines an input variable to the DDL document.
type varDef struct {
...
func readVar(v *varDef, inputVal string) (cty.Value, error) {
	...
	typ, ok := et.(*Type)
	if !ok {
		return cty.NilVal, fmt.Errorf("expected hcl.Type got %T", et)
	}
	switch typ.T {
	case "string":
...
// ctySchemaLit is a cty.Capsule type the encapsulates a hcl.LiteralValue.
var (
	ctySchemaLit = cty.CapsuleWithOps("lit", reflect.TypeOf(LiteralValue{}), &cty.CapsuleOps{
		...
		ConversionFrom: func(src cty.Type) func(any, cty.Path) (cty.Value, error) {
			...
			return func(i any, path cty.Path) (cty.Value, error) {
				...
				if !ok {
					return cty.Value{}, fmt.Errorf("hcl: expected *hcl.LiteralValue got %T", i)
				}
