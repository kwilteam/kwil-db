...
func tname(t *sqlschema.Table) (string, string) {
	if t.Schema != nil {
...
func ParseSchemaFiles(paths ...string) (*sqlschema.Realm, error) {
...
func typeSpec(t sqlschema.Type) (*hcl.Type, error) {
	if t, ok := t.(*sqlschema.TimeType); ok && t.T != TypeDate {
...
func formatTime() hcl.TypeSpecOption {
	return hcl.WithTypeFormatter(func(t *hcl.Type) (string, error) {
		...
		return FormatType(&sqlschema.TimeType{T: t.T, Precision: &p})
	})
}
