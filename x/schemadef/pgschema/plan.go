...
func NewPlanner() sqlschema.Planner {
	return &planner{}
}

// PlanChanges returns a migration plan for the given schema changes.
func (p *planner) PlanChanges(changes []sqlschema.SchemaChange, opts ...sqlschema.PlanOption) (*sqlschema.Plan, error) {
	var planOpts sqlschema.PlanOptions
	for _, o := range opts {
		o(&planOpts)
	}
	s := &state{
		...
		created:     make(map[string]*sqlschema.EnumType),
		altered:     make(map[string]*sqlschema.EnumType),
		dropped:     make(map[string]*sqlschema.EnumType),
...
type state struct {
	sqlschema.Plan
	sqlschema.PlanOptions
	// Track the enums that were created, altered and
	// dropped, in this phase to avoid duplicate updates.
	created, altered, dropped map[string]*sqlschema.EnumType
}

// Exec executes the changes on the database. An error is returned
// if one of the operations fail, or a change is not supported.
func (s *state) plan(changes []sqlschema.SchemaChange) error {
	if s.SchemaQualifier != nil {
		if err := sqlschema.CheckChangesScope(changes); err != nil {
			return err
		}
	}
	planned := s.topLevel(changes)
	planned, err := sqlschema.DetachCycles(planned)
	if err != nil {
		return err
	}
	for _, c := range planned {
		switch c := c.(type) {
		case *sqlschema.AddTable:
			err = s.addTable(c)
		case *sqlschema.DropTable:
			s.dropTable(c)
		case *sqlschema.ModifyTable:
			err = s.modifyTable(c)
		case *sqlschema.RenameTable:
			s.renameTable(c)
		default:
...
func (s *state) topLevel(changes []sqlschema.SchemaChange) []sqlschema.SchemaChange {
	planned := make([]sqlschema.SchemaChange, 0, len(changes))
	for _, c := range changes {
		switch c := c.(type) {
		case *sqlschema.AddSchema:
			b := s.Build("CREATE SCHEMA")
			if sqlschema.Has(c.Extra, &sqlschema.IfNotExists{}) {
				b.P("IF NOT EXISTS")
			}
			b.Ident(c.S.Name)
			s.append(&sqlschema.Change{
				Cmd:     b.String(),
				Source:  c,
				Reverse: s.Build("DROP SCHEMA").Ident(c.S.Name).P("CASCADE").String(),
				Comment: fmt.Sprintf("Add new schema named %q", c.S.Name),
			})
		case *sqlschema.DropSchema:
			b := s.Build("DROP SCHEMA")
			if sqlschema.Has(c.Extra, &sqlschema.IfExists{}) {
				b.P("IF EXISTS")
			}
...
// addTable builds and executes the query for creating a table in a sqlschema.
func (s *state) addTable(add *sqlschema.AddTable) error {
	...
	if sqlschema.Has(add.Extra, &sqlschema.IfNotExists{}) {
		b.P("IF NOT EXISTS")
	}
	b.Table(tname(add.T))
	...
	if p := (Partition{}); sqlschema.Has(add.T.Attrs, &p) {
...
// dropTable builds and executes the query for dropping a table from a sqlschema.
func (s *state) dropTable(drop *sqlschema.DropTable) {
	b := s.Build("DROP TABLE")
	if sqlschema.Has(drop.Extra, &sqlschema.IfExists{}) {
		b.P("IF EXISTS")
	}
...
func (s *state) modifyTable(modify *sqlschema.ModifyTable) error {
	var (
		alter       []sqlschema.SchemaChange
		addI, dropI []*sqlschema.Index
		changes     []*sqlschema.Change
	)
	for _, change := range skipAutoChanges(modify.Changes) {
		switch change := change.(type) {
		case *sqlschema.AddAttr, *sqlschema.ModifyAttr:
			from, to, err := commentChange(change)
			if err != nil {
				return err
			}
			changes = append(changes, s.tableComment(modify.T, to, from))
		case *sqlschema.DropAttr:
			return fmt.Errorf("unsupported change type: %T", change)
		case *sqlschema.AddIndex:
			if c := (sqlschema.Comment{}); sqlschema.Has(change.I.Attrs, &c) {
		...
		case *sqlschema.ModifyIndex:
			k := change.Change
			if change.Change.Is(sqlschema.ChangeComment) {
				from, to, err := commentChange(sqlschema.CommentDiff(change.From.Attrs, change.To.Attrs))
				if err != nil {
					return err
				}
				changes = append(changes, s.indexComment(modify.T, change.To, to, from))
				// If only the comment of the index was changed.
				if k &= ^sqlschema.ChangeComment; k.Is(sqlschema.NoChange) {
					continue
				}
			}
		...
		case *sqlschema.RenameIndex:
			changes = append(changes, &sqlschema.Change{
				Source:  change,
		...
		case *sqlschema.AddColumn:
			...
			if c := (sqlschema.Comment{}); sqlschema.Has(change.C.Attrs, &c) {
				changes = append(changes, s.columnComment(modify.T, change.C, c.Text, ""))
			}
			alter = append(alter, change)
		case *sqlschema.ModifyColumn:
			k := change.Change
			if change.Change.Is(sqlschema.ChangeComment) {
				from, to, err := commentChange(sqlschema.CommentDiff(change.From.Attrs, change.To.Attrs))
				if err != nil {
			...
			alter = append(alter, &sqlschema.ModifyColumn{To: change.To, From: change.From, Change: k})
...
func (s *state) alterTable(t *sqlschema.Table, changes []sqlschema.SchemaChange) error {
	var (
		reverse    []sqlschema.SchemaChange
		reversible = true
	)

	build := func(alter *alterChange, changes []sqlschema.SchemaChange) (string, error) {
		...
		err := b.MapCommaErr(changes, func(i int, b *sqlutil.Builder) error {
			...
			case *sqlschema.AddColumn:
				...
				reverse = append(reverse, &sqlschema.DropColumn{C: change.C})
			case *sqlschema.ModifyColumn:
				if err := s.alterColumn(b, alter, t, change); err != nil {
					return err
				}
				if change.Change.Is(sqlschema.ChangeGenerated) {
					reversible = false
				}
				reverse = append(reverse, &sqlschema.ModifyColumn{
					From:   change.To,
					To:     change.From,
					Change: change.Change & ^sqlschema.ChangeGenerated,
				})
			...
			case *sqlschema.DropColumn:
				...
				reverse = append(reverse, &sqlschema.AddColumn{C: change.C})
			...
			case *sqlschema.DropIndex:
				...
				reverse = append(reverse, &sqlschema.AddIndex{I: change.I})
			case *sqlschema.AddForeignKey:
				b.P("ADD")
				s.fks(b, change.F)
				reverse = append(reverse, &sqlschema.DropForeignKey{F: change.F})
			case *sqlschema.DropForeignKey:
				b.P("DROP CONSTRAINT").Ident(change.F.Name)
				reverse = append(reverse, &sqlschema.AddForeignKey{F: change.F})
			case *sqlschema.AddCheck:
				...
				if reversible = reversible && change.C.Name != ""; reversible {
					reverse = append(reverse, &sqlschema.DropCheck{C: change.C})
				}
			case *sqlschema.DropCheck:
				b.P("DROP CONSTRAINT").Ident(change.C.Name)
				reverse = append(reverse, &sqlschema.AddCheck{C: change.C})
			case *sqlschema.ModifyCheck:
				switch {
				...
				case change.From.Expr != change.To.Expr,
					sqlschema.Has(change.From.Attrs, &NoInherit{}) && !sqlschema.Has(change.To.Attrs, &NoInherit{}),
					!sqlschema.Has(change.From.Attrs, &NoInherit{}) && sqlschema.Has(change.To.Attrs, &NoInherit{}):
				...
				reverse = append(reverse, &sqlschema.ModifyCheck{
					From: change.To,
					To:   change.From,
				})
			}
	...
	cmd.main = &sqlschema.Change{
		Cmd: stmt,
		Source: &sqlschema.ModifyTable{
			T:       t,
			Changes: changes,
		},
	...
	if reversible {
		...
		sqlschema.ReverseChanges(reverse)
...
// alterChange describes an alter table sqlschema.Change where its main command
// can be supported by additional statements before and after it is executed.
type alterChange struct {
	main          *sqlschema.Change
	before, after []*sqlschema.Change
}
...
func (s *state) alterColumn(b *sqlutil.Builder, alter *alterChange, t *sqlschema.Table, c *sqlschema.ModifyColumn) error {
	for k := c.Change; !k.Is(sqlschema.NoChange); {
		b.P("ALTER COLUMN").Ident(c.To.Name)
		switch {
		case k.Is(sqlschema.ChangeType):
			if err := s.alterType(b, alter, t, c); err != nil {
				return err
			}
			k &= ^sqlschema.ChangeType
		case k.Is(sqlschema.ChangeNullability) && c.To.Type.Nullable:
			if t, ok := c.To.Type.Type.(*SerialType); ok {
				return fmt.Errorf("NOT NULL constraint is required for %s column %q", t.T, c.To.Name)
			}
			b.P("DROP NOT NULL")
			k &= ^sqlschema.ChangeNullability
		case k.Is(sqlschema.ChangeNullability) && !c.To.Type.Nullable:
			b.P("SET NOT NULL")
			k &= ^sqlschema.ChangeNullability
		case k.Is(sqlschema.ChangeDefault) && c.To.Default == nil:
			b.P("DROP DEFAULT")
			k &= ^sqlschema.ChangeDefault
		case k.Is(sqlschema.ChangeDefault) && c.To.Default != nil:
			s.columnDefault(b.P("SET"), c.To)
			k &= ^sqlschema.ChangeDefault
		case k.Is(sqlschema.ChangeAttr):
			...
			k &= ^sqlschema.ChangeAttr
		case k.Is(sqlschema.ChangeGenerated):
			if sqlschema.Has(c.To.Attrs, &sqlschema.GeneratedExpr{}) {
				return fmt.Errorf("unexpected generation expression change (expect DROP EXPRESSION): %v", c.To.Attrs)
			}
			b.P("DROP EXPRESSION")
			k &= ^sqlschema.ChangeGenerated
		default: // e.g. sqlschema.ChangeComment.
			return fmt.Errorf("unexpected column change: %d", k)
		}
		if !k.Is(sqlschema.NoChange) {
			b.Comma()
		}
	}
	return nil
...
func (s *state) alterType(b *sqlutil.Builder, alter *alterChange, t *sqlschema.Table, c *sqlschema.ModifyColumn) error {
	...
	case fromHas && !toHas:
		...
		alter.after = append(alter.after, &sqlschema.Change{
			Source:  c,
	...
	case !fromHas && toHas:
		...
		alter.before = append(alter.before, &sqlschema.Change{
			Source:  c,
	...
	default:
		...
		if e, ok := c.To.Type.Type.(*sqlschema.EnumType); ok {
	...
	if collate := (sqlschema.Collation{}); sqlschema.Has(c.To.Attrs, &collate) {
		b.P("COLLATE", collate.V)
	}
	return nil
}

func (s *state) renameTable(c *sqlschema.RenameTable) {
	s.append(&sqlschema.Change{
		Source:  c,
...
func (s *state) addComments(t *sqlschema.Table) {
	var c sqlschema.Comment
	if sqlschema.Has(t.Attrs, &c) && c.Text != "" {
		s.append(s.tableComment(t, c.Text, ""))
	}
	for i := range t.Columns {
		if sqlschema.Has(t.Columns[i].Attrs, &c) && c.Text != "" {
			s.append(s.columnComment(t, t.Columns[i], c.Text, ""))
		}
	}
	for i := range t.Indexes {
		if sqlschema.Has(t.Indexes[i].Attrs, &c) && c.Text != "" {
			s.append(s.indexComment(t, t.Indexes[i], c.Text, ""))
		}
	}
}

func (s *state) tableComment(t *sqlschema.Table, to, from string) *sqlschema.Change {
	b := s.Build("COMMENT ON TABLE").Table(tname(t)).P("IS")
	return &sqlschema.Change{
...
func (s *state) columnComment(t *sqlschema.Table, c *sqlschema.Column, to, from string) *sqlschema.Change {
...
func (s *state) indexComment(t *sqlschema.Table, idx *sqlschema.Index, to, from string) *sqlschema.Change {
...
func (s *state) dropIndexes(t *sqlschema.Table, indexes ...*sqlschema.Index) {
	rs := &state{}
...
func (s *state) mayAddEnums(t *sqlschema.Table, columns ...*sqlschema.Column) error {
...
func (s *state) alterEnum(t *sqlschema.Table, from, to *sqlschema.EnumType) error {
...
// func (s *state) enumExists(ctx context.Context, ns *sqlschema.Schema, e *sqlschema.EnumType) (bool, error) {
...
// mayDropEnum drops dangling enum types form the sqlschema.
func (s *state) mayDropEnum(alter *alterChange, ns *sqlschema.Schema, e *sqlschema.EnumType) error {
	name := s.enumIdent(ns, e)
	if _, ok := s.dropped[name]; ok {
		return nil
	}
	schemas := []*sqlschema.Schema{ns}
...
func (s *state) addIndexes(t *sqlschema.Table, indexes ...*sqlschema.Index) {
...
func (s *state) column(b *sqlutil.Builder, t *sqlschema.Table, c *sqlschema.Column) error {
	...
	for _, attr := range c.Attrs {
		...
		case *Identity, *sqlschema.GeneratedExpr:
			// Handled below.
		default:
			return fmt.Errorf("unexpected column attribute: %T", attr)
		}
	}
	switch hasI, hasX := sqlschema.Has(c.Attrs, &Identity{}), sqlschema.Has(c.Attrs, &sqlschema.GeneratedExpr{}); {
	case hasI && hasX:
	...
	case hasX:
		x := &sqlschema.GeneratedExpr{}
		sqlschema.Has(c.Attrs, x)
...
func (s *state) columnDefault(b *sqlutil.Builder, c *sqlschema.Column) {
	switch x := c.Default.(type) {
	case *sqlschema.Literal:
		v := x.V
		switch c.Type.Type.(type) {
		case *sqlschema.BoolType, *sqlschema.DecimalType, *sqlschema.IntegerType, *sqlschema.FloatType:
		default:
			v = quote(v)
		}
		b.P("DEFAULT", v)
...
func (s *state) indexParts(b *sqlutil.Builder, parts []*sqlschema.IndexPart) {
	b.Wrap(func(b *sqlutil.Builder) {
		b.MapComma(parts, func(i int, b *sqlutil.Builder) {
			...
			case part.Expr != nil:
				b.WriteString(sqlutil.MayWrap(part.Expr.(*sqlschema.RawExpr).X))
			}
...
func (s *state) partAttrs(b *sqlutil.Builder, p *sqlschema.IndexPart) {
	if p.Descending {
		b.P("DESC")
	}
...
func (s *state) index(b *sqlutil.Builder, idx *sqlschema.Index) {
	// Avoid appending the default method.
	if t := (IndexType{}); sqlschema.Has(idx.Attrs, &t) && strings.ToUpper(t.T) != IndexTypeBTree {
		b.P("USING", t.T)
	}
	s.indexParts(b, idx.Parts)
	if c := (IndexInclude{}); sqlschema.Has(idx.Attrs, &c) {
		b.P("INCLUDE")
		b.Wrap(func(b *sqlutil.Builder) {
	...
	if p := (IndexPredicate{}); sqlschema.Has(idx.Attrs, &p) {
		b.P("WHERE").P(p.Predicate)
	}
	for _, attr := range idx.Attrs {
		switch attr.(type) {
		case *sqlschema.Comment, *ConstraintType, *IndexType, *IndexInclude, *IndexPredicate, *IndexStorageParams:
		default:
...
func (s *state) fks(b *sqlutil.Builder, fks ...*sqlschema.ForeignKey) {
...
func (s *state) append(c ...*sqlschema.Change) {
...
func skipAutoChanges(changes []sqlschema.SchemaChange) []sqlschema.SchemaChange {
	var (
		dropC   = make(map[string]bool)
		planned = make([]sqlschema.SchemaChange, 0, len(changes))
	)
	for _, c := range changes {
		if c, ok := c.(*sqlschema.DropColumn); ok {
			dropC[c.C.Name] = true
		}
	}
search:
...
func commentChange(c sqlschema.SchemaChange) (from, to string, err error) {
	switch c := c.(type) {
	case *sqlschema.AddAttr:
		toC, ok := c.A.(*sqlschema.Comment)
		if ok {
			to = toC.Text
			return
		}
		err = fmt.Errorf("unexpected AddAttr.(%T) for comment change", c.A)
	case *sqlschema.ModifyAttr:
		fromC, ok1 := c.From.(*sqlschema.Comment)
		toC, ok2 := c.To.(*sqlschema.Comment)
		if ok1 && ok2 {
...
func check(b *sqlutil.Builder, c *sqlschema.Check) {
	if c.Name != "" {
		b.P("CONSTRAINT").Ident(c.Name)
	}
	b.P("CHECK", sqlutil.MayWrap(c.Expr))
	if sqlschema.Has(c.Attrs, &NoInherit{}) {
		b.P("NO INHERIT")
	}
}

// isUniqueConstraint reports if the index is a valid UNIQUE constraint.
func isUniqueConstraint(i *sqlschema.Index) bool {
	if c := (ConstraintType{}); !sqlschema.Has(i.Attrs, &c) || !c.IsUnique() || !i.Unique {
		return false
	}
...
func (s *state) createDropEnum(sc *sqlschema.Schema, e *sqlschema.EnumType) (string, string) {
...
func (s *state) enumIdent(ns *sqlschema.Schema, e *sqlschema.EnumType) string {
...
func (s *state) enumSchema(ns *sqlschema.Schema, e *sqlschema.EnumType) (es string) {
	switch {
...
func (s *state) schemaPrefix(ns *sqlschema.Schema) string {
	switch {
	case s.SchemaQualifier != nil:
...
func (s *state) formatType(t *sqlschema.Table, c *sqlschema.Column) (string, error) {
	switch tt := c.Type.Type.(type) {
	case *sqlschema.EnumType:
		return s.enumIdent(t.Schema, tt), nil
	case *ArrayType:
		if e, ok := tt.Type.(*sqlschema.EnumType); ok {
...
func hasEnumType(c *sqlschema.Column) (*sqlschema.EnumType, bool) {
	switch t := c.Type.Type.(type) {
	case *sqlschema.EnumType:
		return t, true
	case *ArrayType:
		if e, ok := t.Type.(*sqlschema.EnumType); ok {
			return e, true
		}
	}
	return nil, false
