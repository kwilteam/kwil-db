...
// A diff provides a PostgreSQL implementation for sqlschema.Inspector.
type inspect struct{ conn }

var _ sqlschema.Inspector = (*inspect)(nil)

// InspectRealm returns schema descriptions of all resources in the given realm.
func (i *inspect) InspectRealm(ctx context.Context, opts *sqlschema.InspectRealmOption) (*sqlschema.Realm, error) {
	...
	r := sqlschema.NewRealm(schemas...).SetCollation(i.collate)
	r.Attrs = append(r.Attrs, &CType{V: i.ctype})
	if len(schemas) == 0 || !sqlschema.ModeInspectRealm(opts).Is(sqlschema.InspectTables) {
		return sqlschema.ExcludeRealm(r, opts.Exclude)
	}
	if err := i.inspectTables(ctx, r, nil); err != nil {
		return nil, err
	}
	sqlschema.LinkSchemaTables(schemas)
	return sqlschema.ExcludeRealm(r, opts.Exclude)
}

// InspectSchema returns schema descriptions of the tables in the given sqlschema.
// If the schema name is empty, the result will be the attached sqlschema.
func (i *inspect) InspectSchema(ctx context.Context, name string, opts *sqlschema.InspectOptions) (s *sqlschema.Schema, err error) {
	schemas, err := i.schemas(ctx, &sqlschema.InspectRealmOption{Schemas: []string{name}})
	if err != nil {
		return nil, err
	}
	...
	if opts == nil {
		opts = &sqlschema.InspectOptions{}
	}
	r := sqlschema.NewRealm(schemas...).SetCollation(i.collate)
	r.Attrs = append(r.Attrs, &CType{V: i.ctype})
	if sqlschema.ModeInspectSchema(opts).Is(sqlschema.InspectTables) {
		if err := i.inspectTables(ctx, r, opts); err != nil {
			return nil, err
		}
		sqlschema.LinkSchemaTables(schemas)
	}
	return sqlschema.ExcludeSchema(r.Schemas[0], opts.Exclude)
}

func (i *inspect) inspectTables(ctx context.Context, r *sqlschema.Realm, opts *sqlschema.InspectOptions) error {
...
func (i *inspect) tables(ctx context.Context, realm *sqlschema.Realm, opts *sqlschema.InspectOptions) error {
	var (
		args  []any
	...
	for rows.Next() {
		...
		t := &sqlschema.Table{Name: name.String}
		s.AddTables(t)
...
func (i *inspect) columns(ctx context.Context, s *sqlschema.Schema) error {
	query := columnsQuery
...
func (i *inspect) addColumn(s *sqlschema.Schema, rows *sql.Rows) (err error) {
	var (
...
func (i *inspect) enumValues(ctx context.Context, s *sqlschema.Schema) error {
	var (
		args  []any
		ids   = make(map[int64][]*sqlschema.EnumType)
		query = "SELECT enumtypid, enumlabel FROM pg_enum WHERE enumtypid IN (%s)"
		newE  = func(e1 *EnumType) *sqlschema.EnumType {
			if _, ok := ids[e1.ID]; !ok {
				args = append(args, e1.ID)
			}
			// Convert the intermediate type to
			// the standard sqlschema.EnumType.
			e2 := &sqlschema.EnumType{T: e1.T, Schema: s}
			if e1.Schema != "" && e1.Schema != s.Name {
				e2.Schema = sqlschema.New(e1.Schema)
			}
...
func (i *inspect) indexes(ctx context.Context, s *sqlschema.Schema) error {
	query := indexesQuery
...
func (i *inspect) addIndexes(s *sqlschema.Schema, rows *sql.Rows) error {
	names := make(map[string]*sqlschema.Index)
	for rows.Next() {
		var (
		...
		if !ok {
			...
			if sqlutil.ValidString(comment) {
				idx.Attrs = append(idx.Attrs, &sqlschema.Comment{Text: comment.String})
			}
		...
		part := &sqlschema.IndexPart{Seq: len(idx.Parts) + 1, Descending: desc.Bool}
		...
		case included:
			...
			sqlschema.Has(idx.Attrs, &include)
			include.Columns = append(include.Columns, c.Name)
			sqlschema.ReplaceOrAppend(&idx.Attrs, &include)
...
// partitions builds the partition each table in the sqlschema.
func (i *inspect) partitions(s *sqlschema.Schema) error {
	for _, t := range s.Tables {
		var d Partition
		if !sqlschema.Has(t.Attrs, &d) {
			continue
		}
		...
		for i := range idxs {
			...
			case idx == 0:
				...
				d.Parts = append(d.Parts, &PartitionPart{
					Expr: &sqlschema.RawExpr{X: d.exprs[:j+1]},
				})
...
func (i *inspect) checks(ctx context.Context, s *sqlschema.Schema) error {
...
func (i *inspect) addChecks(s *sqlschema.Schema, rows *sql.Rows) error {
	names := make(map[string]*sqlschema.Check)
	for rows.Next() {
		var (
			noInherit                            bool
		...
		check, ok := names[name]
		if !ok {
			check = &sqlschema.Check{Name: name, Expr: clause, Attrs: []sqlschema.Attr{&CheckColumns{}}}
			if noInherit {
...
func (i *inspect) schemas(ctx context.Context, opts *sqlschema.InspectRealmOption) ([]*sqlschema.Schema, error) {
	var (
		args  []any
		query = schemasQuery
	)
	...
	defer rows.Close()
	var schemas []*sqlschema.Schema
	for rows.Next() {
		var name string
		if err := rows.Scan(&name); err != nil {
			return nil, err
		}
		schemas = append(schemas, &sqlschema.Schema{
			Name: name,
		})
	}
	if err := rows.Close(); err != nil {
...
func (i *inspect) querySchema(ctx context.Context, query string, s *sqlschema.Schema) (*sql.Rows, error) {
	args := []any{s.Name}
	for _, t := range s.Tables {
...
func defaultExpr(c *sqlschema.Column, s string) {
	...
	case len(m) == 2:
		tt, ok := c.Type.Type.(*sqlschema.IntegerType)
		if !ok {
			return
		}
	...
	case sqlutil.IsLiteralBool(s), sqlutil.IsLiteralNumber(s), sqlutil.IsQuoted(s, '\''):
		c.Default = &sqlschema.Literal{V: s}
	default:
		var x sqlschema.Expr = &sqlschema.RawExpr{X: s}
		// Try casting or fallback to raw expressions (e.g. column text[] has the default of '{}':text[]).
		if v, ok := canConvert(c.Type, s); ok {
			x = &sqlschema.Literal{V: v}
		}
		c.Default = x
	}
}

func canConvert(t *sqlschema.ColumnType, x string) (string, bool) {
	...
	case *sqlschema.DecimalType, *sqlschema.IntegerType, *sqlschema.FloatType:
		if sqlutil.IsLiteralNumber(x) {
			return x, true
		}
	case *ArrayType, *sqlschema.BinaryType, *sqlschema.JSONType, *NetworkType, *sqlschema.SpatialType, *sqlschema.StringType, *sqlschema.TimeType, *UUIDType, *XMLType:
		return q, true
	}
	return "", false
}
...
type (
	...
	ArrayType struct {
		sqlschema.Type        // Underlying items type (e.g. varchar(255)).
...
func (s *SerialType) IntegerType() *sqlschema.IntegerType {
	t := &sqlschema.IntegerType{T: TypeInteger}
	switch s.T {
...
func (s *SerialType) SetType(t *sqlschema.IntegerType) {
	switch t.T {
...
func (s *SerialType) sequence(t *sqlschema.Table, c *sqlschema.Column) string {
	if s.SequenceName != "" {
