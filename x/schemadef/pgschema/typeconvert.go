...
func FormatType(t sqlschema.Type) (string, error) {
	var f string
	switch t := t.(type) {
	case *ArrayType:
...
func GetLength(t sqlschema.Type) (int, bool) {
	switch t := t.(type) {
	case *BitType:
		if t.Size != 0 {
...
// ParseType returns the sqlschema.Type value represented by the given raw type.
...
func ParseType(typ string) (sqlschema.Type, error) {
	var (
		err error
		d   *columnDesc
	)
...
func columnType(c *columnDesc) (sqlschema.Type, error) {
	...
	case TypeTime, TypeTimeWOTZ, TypeTimeTZ, TypeTimeWTZ, TypeTimestamp,
		...
		typ = &sqlschema.TimeType{T: t, Precision: &p}
	case TypeInterval:
		p := defaultTimePrecision
		if c.timePrecision != nil {
			p = int(*c.timePrecision)
		...
		if c.interval != "" {
			...
			if !ok {
				return &sqlschema.UnsupportedType{T: c.interval}, nil
			}
			typ.(*IntervalType).F = f
		}
	case TypeReal, TypeDouble, TypeFloat, TypeFloat4, TypeFloat8:
		typ = &sqlschema.FloatType{T: t, Precision: int(c.precision)}
	case TypeJSON, TypeJSONB:
		typ = &sqlschema.JSONType{T: t}
	case TypeMoney:
		typ = &CurrencyType{T: t}
	case TypeDecimal, TypeNumeric:
		typ = &sqlschema.DecimalType{T: t, Precision: int(c.precision), Scale: int(c.scale)}
	...
	default:
		typ = &sqlschema.UnsupportedType{T: t}
	}
	return typ, nil
}
