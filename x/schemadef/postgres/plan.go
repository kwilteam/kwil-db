...
func NewPlanner() schema.Planner {
	return &planner{}
}

// PlanChanges returns a migration plan for the given schema changes.
func (p *planner) PlanChanges(changes []schema.SchemaChange, opts ...schema.PlanOption) (*schema.Plan, error) {
	var planOpts schema.PlanOptions
	for _, o := range opts {
		o(&planOpts)
	}
	s := &state{
...
func (s *state) plan(changes []schema.SchemaChange) error {
		...
		err := b.MapCommaErr(changes, func(i int, b *sqlutil.Builder) error {
			...
			case *schema.ModifyCheck:
				...
				case change.From.Expr != change.To.Expr,
					schema.Has(change.From.Attrs, &NoInherit{}) && !schema.Has(change.To.Attrs, &NoInherit{}),
					!schema.Has(change.From.Attrs, &NoInherit{}) && schema.Has(change.To.Attrs, &NoInherit{}):
...
// alterChange describes an alter table schema.Change where its main command
...
func (s *state) alterColumn(b *sqlutil.Builder, alter *alterChange, t *schema.Table, c *schema.ModifyColumn) error {
...
func (s *state) alterType(b *sqlutil.Builder, alter *alterChange, t *schema.Table, c *schema.ModifyColumn) error {
...
func (s *state) mayAddEnums(t *schema.Table, columns ...*schema.Column) error {
...
func (s *state) column(b *sqlutil.Builder, t *schema.Table, c *schema.Column) error {
...
func (s *state) columnDefault(b *sqlutil.Builder, c *schema.Column) {
...
func (s *state) indexParts(b *sqlutil.Builder, parts []*schema.IndexPart) {
	b.Wrap(func(b *sqlutil.Builder) {
		b.MapComma(parts, func(i int, b *sqlutil.Builder) {
			...
			case part.Expr != nil:
				b.WriteString(sqlutil.MayWrap(part.Expr.(*schema.RawExpr).X))
			}
...
func (s *state) partAttrs(b *sqlutil.Builder, p *schema.IndexPart) {
	if p.Descending {
		b.P("DESC")
	}
...
func (s *state) index(b *sqlutil.Builder, idx *schema.Index) {
...
func (s *state) fks(b *sqlutil.Builder, fks ...*schema.ForeignKey) {
...
func check(b *sqlutil.Builder, c *schema.Check) {
	if c.Name != "" {
		b.P("CONSTRAINT").Ident(c.Name)
	}
	b.P("CHECK", sqlutil.MayWrap(c.Expr))
	if schema.Has(c.Attrs, &NoInherit{}) {
		b.P("NO INHERIT")
	}
}
