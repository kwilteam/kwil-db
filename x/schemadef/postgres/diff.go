...
func NewDiffer() schema.Differ {
	return &schema.Diff{DiffDriver: &diff{}}
}
...
func (d *diff) TableAttrDiff(from, to *schema.Table) ([]schema.SchemaChange, error) {
	...
	return append(changes, schema.CheckDiff(from, to, func(c1, c2 *schema.Check) bool {
		return schema.Has(c1.Attrs, &NoInherit{}) == schema.Has(c2.Attrs, &NoInherit{})
	})...), nil
}
...
func (*diff) IndexAttrChanged(from, to []schema.Attr) bool {
	...
	if schema.Has(from, &p1) != schema.Has(to, &p2) || (p1.Predicate != p2.Predicate && p1.Predicate != sqlutil.MayWrap(p2.Predicate)) {
		return true
	}
	...
	b.Wrap(func(b *sqlutil.Builder) {
		b.MapComma(p.Parts, func(i int, b *sqlutil.Builder) {
			...
			case k.Column != "":
				b.Ident(k.Column)
			case k.Expr != nil:
				b.P(sqlutil.MayWrap(k.Expr.(*schema.RawExpr).X))
			}
		})
	})
	return b.String(), nil
