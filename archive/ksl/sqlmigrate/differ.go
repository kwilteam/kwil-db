...
type Differ interface {
	Diff(from, to sqlschema.Database) ([]MigrationStep, error)
}
...
func (d *differ) Diff(from, to sqlschema.Database) ([]MigrationStep, error) {
...
type diffctx struct {
	Schemas ty.Pair[sqlschema.Database]
	Db      *diffdb
	Steps   []MigrationStep
}

func newDiffContext(prev, next sqlschema.Database, flavor SqlDiffFlavor) *diffctx {
	return &diffctx{
		Db:      newDiffDb(prev, next, flavor),
		Schemas: ty.MakePair(prev, next),
	}
}
...
func (ctx *diffctx) calculateCreatedIndexSteps() error {
	for _, table := range ctx.Db.TablePairs() {
		...
		var darcols []sqlschema.ColumnID
		for _, col := range table.ColumnPairs() {
			changes := ctx.Db.ColumnChanges[ty.MakePair(col.Prev.ID, col.Next.ID)]
	...
	for _, table := range ctx.Db.TablePairs() {
		...
		for _, fk := range table.ForeignKeyPairs() {
			if fk.Prev.ConstraintName() != fk.Next.ConstraintName() {
				...
				ctx.step(RenameForeignKey{ForeignKeys: ty.MakePair(fk.Prev.ID, fk.Next.ID)})
			}
		}

		for _, index := range table.IndexPairs() {
			if index.Prev.Name() != index.Next.Name() {
				ctx.step(RenameIndex{Index: ty.MakePair(index.Prev.ID, index.Next.ID)})
			}
		}
		...
		for _, col := range table.ColumnPairs() {
			columnChanges := ctx.Db.ColumnChanges[ty.MakePair(col.Prev.ID, col.Next.ID)]

			if !columnChanges.DiffersInSomething() {
				continue
			}

			colid := ty.MakePair(col.Prev.ID, col.Next.ID)
		...
		ctx.step(AlterTable{Tables: ty.MakePair(table.Previous().ID, table.Next().ID), Changes: changes})
	}
