...
func Test_Planner(t *testing.T) {
	...
	tests := []testcase{
		...
		// 		"└─Projection: $a.b; $c AS c1\n" +
		// 		"  └─Empty Scan\n",
		// },
		// {
		// 	name: "select with filter",
		// 	sql:  "select id, name from users where age > 18",
		// 	wt: "Return: id [uuid] (from users.id), name [text] (from users.name)\n" +
		...
		// 	wt: "Return: name [text] (from u.name)\n" +
		// 		"└─Projection: u.name\n" +
		...
		// 	wt: "Return: name [text] (from users.name)\n" +
		...
		// 	wt: "Return: sum [decimal(1000,0)] (from #ref(A))\n" +
		// 		"└─Projection: {#ref(A)}\n" +
		...
		// 	wt: "Return: name [text] (from #ref(A)), sum [decimal(1000,0)] (from #ref(B))\n" +
		// 		"└─Projection: {#ref(A)}; {#ref(B)}\n" +
		...
		// 	wt: "Return: res [int]\n" +
		// 		"└─Projection: {#ref(B)}::int / {#ref(A)} AS res\n" +
		...
		// 	wt: "Return: ?column? [int] (from #ref(A)), ?column? [int] (from #ref(B))\n" +
		// 		"└─Projection: {#ref(A)}; {#ref(B)}\n" +
		// 		"  └─Aggregate [{#ref(B) = users.age * 3}] [{#ref(A) = users.age / 2}]\n" +
		// 		"    └─Scan Table: users [physical]\n",
		// },
		// // TODO: negative case of the above
		// {
		...
		// 	wt: "Return: id [uuid] (from #ref(C)), name [text] (from #ref(A)), age [int] (from #ref(B))\n" +
		// 		"└─Projection: {#ref(C)}; {#ref(A)}; {#ref(B)}\n" +
		// 		"  └─Aggregate [{#ref(B) = users.age}] [{#ref(C) = users.id}] [{#ref(A) = users.name}]\n" +
		// 		"    └─Scan Table: users [physical]\n",
		// },
		// {
		...
		// 	// TODO: we don't have a way of tracking the source of an addition of two aggregates
		// 	// I am not too worried about it rn since it will probably be a problem I have to solve
		// 	// when I get to the optimizer, but leaving a note here in case it doesn't come up.
		// 	// EDIT: Im not sure if this is even necessary at all if the relations match (which they do)
		// 	wt: "Return: name [text] (from #ref(A)), ?column? [decimal(1000,0)]\n" +
		// 		"└─Projection: {#ref(A)}; {#ref(C)} + {#ref(D)}\n" +
		...
		// 	wt: "Return: name [text] (from #ref(A)), age [int] (from #ref(B))\n" +
		// 		"└─Projection: {#ref(A)}; {#ref(B)}\n" +
		// 		"  └─Aggregate [{#ref(B) = users.age}] [{#ref(A) = users.name}]\n" +
		// 		"    └─Scan Table: users [physical]\n",
		// },
		{
		...
		// 	wt: "Return: brand [text] (from #ref(A)), content [text] (from #ref(B)), name [text] (from #ref(C)), id [uuid] (from #ref(D)), count [int] (from #ref(E))\n" +
		// 		"└─Projection: {#ref(A)}; {#ref(B)}; {#ref(C)}; {#ref(D)}; {#ref(E)}\n" +
		...
		// 		b as (select * from a)
		// 		select * from b;`,
		// 	wt: "Return: id2 [uuid] (from b.id2), name2 [text] (from b.name2)\n" +
		...
		// 	wt: "Return: name [text] (from users.name), age [int] (from users.age)\n" +
		...
		// 	wt: "Return: count [int] (from #ref(A)), sum [decimal(1000,0)] (from #ref(B))\n" +
		// 		"└─Projection: {#ref(A)}; {#ref(B)}\n" +
