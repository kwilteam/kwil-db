...
foreign_declaration:
    FOREIGN_PROCEDURE name=IDENTIFIER
    LPAREN (unnamed_params=type_selector_list|named_params=typed_variable_list)? RPAREN
    (RETURNS (TABLE? LPAREN return_columns=named_type_list RPAREN
    | LPAREN unnamed_return_types=type_selector_list RPAREN))?
;
...
type_selector_list:
    type_selector (COMMA type_selector)*
;

named_type_list:
    IDENTIFIER type_selector (COMMA IDENTIFIER type_selector)*
;

typed_variable_list:
    VAR type_selector (COMMA VAR type_selector)*
;
