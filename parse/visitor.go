...
func (s *schemaVisitor) VisitInteger_literal(ctx *gen.Integer_literalContext) any {
	...
	if !ok {
		s.errs.RuleErr(ctx, parseTypes.ParseErrorTypeSyntax, errors.New("invalid integer literal"))
	...
	if bigNum.Cmp(minInt64) < 0 {
		s.errs.RuleErr(ctx, parseTypes.ParseErrorTypeSemantic, errors.New("negative integer literal out of range"))
		return unknownExpression(ctx)
	}
...
func (s *schemaVisitor) VisitDecimal_literal(ctx *gen.Decimal_literalContext) any {
	...
	if err != nil {
		s.errs.RuleErr(ctx, parseTypes.ParseErrorTypeSemantic, err)
		return unknownExpression(ctx)
	}
...
func (s *schemaVisitor) VisitBinary_literal(ctx *gen.Binary_literalContext) any {
	...
	if b[:2] != "0x" {
		...
		s.errs.RuleErr(ctx, parseTypes.ParseErrorTypeSyntax, errors.New("invalid blob literal"))
	}
...
func (s *schemaVisitor) VisitType(ctx *gen.TypeContext) any {
	...
	if ctx.LPAREN() != nil {
		...
		if err != nil {
			s.errs.RuleErr(ctx, parseTypes.ParseErrorTypeSyntax, err)
...
func (s *schemaVisitor) VisitSchema(ctx *gen.SchemaContext) any {
	...
	for i, t := range ctx.AllTable_declaration() {
		s.schema.Tables = append(s.schema.Tables, t.Accept(s).(*types.Table))
		s.registerBlock(t, s.schema.Tables[i].Name)
	}

	for i, e := range ctx.AllUse_declaration() {
		s.schema.Extensions = append(s.schema.Extensions, e.Accept(s).(*types.Extension))
		s.registerBlock(e, s.schema.Extensions[i].Name)
	}

	for i, a := range ctx.AllAction_declaration() {
		s.schema.Actions = append(s.schema.Actions, a.Accept(s).(*types.Action))
		s.registerBlock(a, s.schema.Actions[i].Name)
	}

	for i, p := range ctx.AllProcedure_declaration() {
		s.schema.Procedures = append(s.schema.Procedures, p.Accept(s).(*types.Procedure))
		s.registerBlock(p, s.schema.Procedures[i].Name)
	}

	for i, p := range ctx.AllForeign_procedure_declaration() {
		s.schema.ForeignProcedures = append(s.schema.ForeignProcedures, p.Accept(s).(*types.ForeignProcedure))
...
func (s *schemaVisitor) registerBlock(ctx antlr.ParserRuleContext, name string) {
	...
	if _, ok := s.schemaInfo.Blocks[lower]; ok {
		s.errs.RuleErr(ctx, parseTypes.ParseErrorTypeSemantic, errors.New("duplicate block name"))
		return
	}

	node := &parseTypes.Node{}
	node.Set(ctx)
	s.schemaInfo.Blocks[lower] = &parseTypes.Block{
		Node:     *node,
...
func (s *schemaVisitor) VisitAnnotation(ctx *gen.AnnotationContext) any {
	// we simply return all text here because core Kwil does not care
	// about annotations
	return ctx.GetText()
}
...
func (s *schemaVisitor) VisitUse_declaration(ctx *gen.Use_declarationContext) any {
	...
	e := &types.Extension{
		Name:           strings.ToLower(ctx.IDENTIFIER(0).GetText()),
		Initialization: arr[*types.ExtensionConfig](len(ctx.AllIDENTIFIER()) - 2),
		Alias:          ctx.IDENTIFIER(len(ctx.AllIDENTIFIER()) - 1).GetText(),
	}
...
func (s *schemaVisitor) VisitIndex_def(ctx *gen.Index_defContext) any {
	...
	s.validateIdent(ctx.HASH_IDENTIFIER().GetSymbol(), idx.Name)
...
func (s *schemaVisitor) VisitMIN(ctx *gen.MINContext) any {
...
func (s *schemaVisitor) VisitMAX(ctx *gen.MAXContext) any {
	return &types.Attribute{
		Type:  types.MAX,
		Value: ctx.Literal().Accept(s).(*ExpressionLiteral).String(),
	}
...
func (s *schemaVisitor) VisitMIN_LEN(ctx *gen.MIN_LENContext) any {
	return &types.Attribute{
...
func (s *schemaVisitor) VisitMAX_LEN(ctx *gen.MAX_LENContext) any {
	return &types.Attribute{
...
func (s *schemaVisitor) VisitNOT_NULL(ctx *gen.NOT_NULLContext) any {
	return &types.Attribute{
		Type: types.NOT_NULL,
	}
}

func (s *schemaVisitor) VisitPRIMARY_KEY(ctx *gen.PRIMARY_KEYContext) any {
	return &types.Attribute{
		Type: types.PRIMARY_KEY,
	}
}

func (s *schemaVisitor) VisitDEFAULT(ctx *gen.DEFAULTContext) any {
	return &types.Attribute{
		Type:  types.DEFAULT,
		Value: ctx.Literal().Accept(s).(*ExpressionLiteral).String(),
	}
...
func (s *schemaVisitor) VisitUNIQUE(ctx *gen.UNIQUEContext) any {
	return &types.Attribute{
		Type: types.UNIQUE,
	}
}
...
func (s *schemaVisitor) VisitAction_declaration(ctx *gen.Action_declarationContext) any {
	...
	// visit the action block to parse it, but simply store the string
	// TODO: we should perform validation here on the passed action.
	// waiting to do this once I have implemented it
	ctx.Action_block().Accept(s)

	act.Body = ctx.Action_block().GetText()
...
func (s *schemaVisitor) VisitProcedure_declaration(ctx *gen.Procedure_declarationContext) any {
	proc := &types.Procedure{
		Name:        s.getIdent(ctx.IDENTIFIER()),
		Annotations: arr[string](len(ctx.AllAnnotation())),
		Parameters:  ctx.Typed_variable_list().Accept(s).([]*types.ProcedureParameter),
		Returns:     ctx.Procedure_return().Accept(s).(*types.ProcedureReturn),
	}
	...
	// visit the procedure block to parse it, but simply store the string
	// TODO: we should perform validation here on the passed procedure.
	// waiting to do this once I have implemented it
	ctx.Procedure_block().Accept(s)

	proc.Body = ctx.Procedure_block().GetText()
...
func (s *schemaVisitor) VisitSelect_core(ctx *gen.Select_coreContext) any {
	...
	if ctx.Relation() != nil {
		stmt.From = ctx.Relation().Accept(s).(Relation)
	}
...
func (s *schemaVisitor) VisitJoin(ctx *gen.JoinContext) any {
	j := &Join{
		Relation: ctx.Relation().Accept(s).(Relation),
...
func (s *schemaVisitor) VisitUpdate_statement(ctx *gen.Update_statementContext) any {
	...
	if ctx.GetAlias() != nil {
		up.Alias = strings.ToLower(ctx.GetAlias().GetText())
	}
	...
	if ctx.Relation() != nil {
		up.From = ctx.Relation().Accept(s).(Relation)
	}
...
func (s *schemaVisitor) VisitInsert_statement(ctx *gen.Insert_statementContext) any {
	...
	if ctx.GetAlias() != nil {
		ins.Alias = strings.ToLower(ctx.GetAlias().GetText())
	}
...
func (s *schemaVisitor) VisitDelete_statement(ctx *gen.Delete_statementContext) any {
	...
	if ctx.GetAlias() != nil {
		d.Alias = strings.ToLower(ctx.GetAlias().GetText())
	}
...
func (s *schemaVisitor) VisitColumn_sql_expr(ctx *gen.Column_sql_exprContext) any {
	e := &ExpressionColumn{
		Column: strings.ToLower(ctx.GetColumn().GetText()),
	}

	if ctx.GetTable() != nil {
		e.Table = strings.ToLower(ctx.GetTable().GetText())
	}
...
func (s *schemaVisitor) VisitForeign_call_sql(ctx *gen.Foreign_call_sqlContext) any {
	e := &ExpressionForeignCall{
		Name: s.getIdent(ctx.IDENTIFIER()),
		Args: ctx.Sql_expr_list().Accept(s).([]Expression),
	}
...
func (s *schemaVisitor) VisitExtension_action(ctx *gen.Extension_actionContext) any {
	...
	if ctx.Variable_list() != nil {
		stmt.Receivers = ctx.Variable_list().Accept(s).([]string)
	}
...
func (s *schemaVisitor) VisitStmt_for_loop(ctx *gen.Stmt_for_loopContext) any {
	...
	for i, st := range ctx.AllStatement() {
...
func (s *schemaVisitor) VisitStmt_if(ctx *gen.Stmt_ifContext) any {
	stmt := &ProcedureStmtIf{
		...
		Else:    arr[ProcedureStmt](len(ctx.AllStatement())),
	}
...
func (s *schemaVisitor) getIdent(i antlr.TerminalNode) string {
	ident := i.GetText()
	s.validateIdent(i.GetSymbol(), ident)
	return strings.ToLower(ident)
}

func (s *schemaVisitor) validateIdent(i antlr.Token, str string) {
	if len(str) > maxIdentifierLength {
		s.errs.TokenErr(i, parseTypes.ParseErrorTypeSyntax, errors.New("identifier too long"))
	}
}
