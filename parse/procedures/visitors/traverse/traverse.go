...
import parser "github.com/kwilteam/kwil-db/parse/procedures/parser"
...
func (v *Traverser) VisitExpressionForeignCall(s *parser.ExpressionForeignCall) interface{} {
	if v.ExpressionForeignCall != nil {
		v.ExpressionForeignCall(s)
	}

	for _, arg := range s.ContextArgs {
		arg.Accept(v)
	}
