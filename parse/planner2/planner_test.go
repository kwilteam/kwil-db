...
import (
	...
	"github.com/kwilteam/kwil-db/parse/planner2"
...
func Test_Planner(t *testing.T) {
	...
	tests := []testcase{
		...
		},
		{
			...
			err:  planner2.ErrIllegalAggregate,
		},
		{
			name: "complex having",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			plan, err := planner2.Plan(parsedSql.AST, schema, test.vars, test.objects)
			if test.err != nil {
				require.Error(t, err)
