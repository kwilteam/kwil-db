...
func (t *typeVisitor) VisitCTE(p0 *tree.CTE) any {
	return evalFunc(func(e *evaluationContext) {
		...
		if ok {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("common table expression conflicts with existing table %s", p0.Table))
			return
		}
...
func (t *typeVisitor) VisitRelationFunction(p0 *tree.RelationFunction) any {
	...
	return evalFunc(func(e *evaluationContext) {
		...
		parameters, returns, err := util.FindProcOrForeign(t.Schema, p0.Function.Function)
		if err != nil {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, err)
			return
		}
		...
		if len(p0.Function.Inputs) != len(parameters) {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("procedure %s expected %d inputs, received %d", p0.Function.Function, len(parameters), len(p0.Function.Inputs)))
			return
		}
		...
		for i, in := range p0.Function.Inputs {
			...
			if !attr.Type.Equals(parameters[i]) {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("procedure %s expected input %d to be %s, received %s", p0.Function.Function, i, parameters[i].String(), attr.Type.String()))
				// we don't have to return here, we can continue to check the return type
			}
		}

		if returns == nil {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("procedure %s does not return a table", p0.Function.Function))
			return
		}
		if !returns.IsTable {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("procedure %s does not return a table", p0.Function.Function))
...
func (t *typeVisitor) VisitRelationTable(p0 *tree.RelationTable) any {
	return evalFunc(func(e *evaluationContext) {
		...
		if !ok {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("table %s not found", p0.Name))
			return
		}
	...
	return evalFunc(func(e *evaluationContext) {
		...
		if p0.Where != nil {
			...
			if !attr.Type.Equals(types.BoolType) {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("where clause must evaluate to boolean. Received: %s", attr.Type.String()))
				return
			}
		}
	})
...
func (t *typeVisitor) VisitUpdateSetClause(p0 *tree.UpdateSetClause) any {
	return evalFunc(func(e *evaluationContext) {
		...
		if len(e.joinOrder) == 0 {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, errors.New("no table to update"))
			return
		}
	...
	return evalFunc(func(e *evaluationContext) {
		if p0.Expression != nil {
			...
			if !limit.Type.Equals(types.IntType) {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("limit must be an integer. Received: %s", limit.Type.String()))
		...
		if p0.Offset != nil {
			...
			if !offset.Type.Equals(types.IntType) {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("offset must be an integer. Received: %s", offset.Type.String()))
			}
		}
	})
}
...
func (t *typeVisitor) VisitGroupBy(p0 *tree.GroupBy) any {
	return evalFunc(func(e *evaluationContext) {
		...
		if p0.Having != nil {
			...
			if !attr.Type.Equals(types.BoolType) {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("having clause must be boolean. Received: %s", attr.Type.String()))
				return
			}
		}
	})
	...
	return evalFunc(func(e *evaluationContext) {
		...
		if p0.Constraint != nil {
			...
			if !r.Type.Equals(types.BoolType) {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("join constraint must be boolean. Received: %s", r.Type.String()))
			}
		}
	})
}
...
func (t *typeVisitor) VisitExpressionArithmetic(p0 *tree.ExpressionArithmetic) any {
	return attributeFn(func(ev *evaluationContext) *QualifiedAttribute {
		...
		if !at.Type.Equals(types.IntType) {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("arithmetic expression expected int. Received: %s", at.Type.String()))
			return unknownAttr()
		}

		bt := right(ev)
		if !bt.Type.Equals(types.IntType) {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("arithmetic expression expected int. Received: %s", bt.Type.String()))
			return unknownAttr()
		}
...
func (t *typeVisitor) VisitExpressionBetween(p0 *tree.ExpressionBetween) any {
	return attributeFn(func(ev *evaluationContext) *QualifiedAttribute {
		...
		if !et.Type.Equals(lt.Type) {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("between expression expected %s. Received: %s", et.Type.Name, lt.Type.String()))
			return unknownAttr()
		}

		if !et.Type.Equals(rt.Type) {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("between expression expected %s. Received: %s", et.Type.Name, rt.Type.String()))
			return unknownAttr()
		}
...
func (t *typeVisitor) VisitExpressionBinaryComparison(p0 *tree.ExpressionBinaryComparison) any {
	return attributeFn(func(ev *evaluationContext) *QualifiedAttribute {
		...
		if !at.Type.Equals(bt.Type) {
			t.ErrorListener.NodeErr(parseTypes.MergeNodes(p0.Left.GetNode(), p0.Right.GetNode()), parseTypes.ParseErrorTypeType, fmt.Errorf("cannot compare types: left: %s right: %s", at.Type.String(), bt.Type.String()))
			return unknownAttr()
		}
	...
	return attributeFn(func(ev *evaluationContext) *QualifiedAttribute {
		...
		if !ok {
			if t.ArbitraryBinds {
				...
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("bind parameter %s not found", util.UnformatParameterName(p0.Parameter)))
				return unknownAttr()
			}
		}
	...
	return attributeFn(func(ev *evaluationContext) *QualifiedAttribute {
		...
		for _, w := range p0.WhenThenPairs {
			...
			if !whenType.Type.Equals(expectedWhenType) {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("when clause expected %s. Received: %s", expectedWhenType.String(), whenType.Type.String()))
				return unknownAttr()
			}
			...
			if !neededType.Equals(thenType.Type) {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("all THEN types must be the same. Received: %s and %s", neededType.String(), thenType.Type.String()))
				return unknownAttr()
			}
		}
		...
		if p0.ElseExpression != nil {
			...
			if !neededType.Equals(eType.Type) {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("ELSE type must match THEN type. Received: %s and %s", neededType.String(), eType.Type.String()))
				return unknownAttr()
			}
		}
...
func (t *typeVisitor) VisitExpressionFunction(p0 *tree.ExpressionFunction) any {
	return attributeFn(func(ev *evaluationContext) *QualifiedAttribute {
		...
		if !ok {
			...
			params, returns, err := util.FindProcOrForeign(t.Schema, p0.Function)
			if err != nil {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, err)
				return unknownAttr()
			}
			...
			if len(p0.Inputs) != len(params) {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("procedure %s expected %d inputs, received %d", p0.Function, len(params), len(p0.Inputs)))
				return unknownAttr()
			}
			...
			for i, in := range p0.Inputs {
				...
				if !attr.Type.Equals(params[i]) {
					t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("procedure %s expected input %d to be %s, received %s", p0.Function, i, params[i].String(), attr.Type.String()))
					return unknownAttr()
				}
			}
			...
			if len(returns.Fields) != 1 {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("procedure %s must return exactly one column", p0.Function))
				return unknownAttr()
			}
		...
		if err != nil {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, err)
			return unknownAttr()
		}
...
func (t *typeVisitor) VisitExpressionIs(p0 *tree.ExpressionIs) any {
	return attributeFn(func(ev *evaluationContext) *QualifiedAttribute {
		...
		if !lt.Type.Equals(rt.Type) && !lt.Type.Equals(types.NullType) && !rt.Type.Equals(types.NullType) {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("is expression expected %s. Received: %s", lt.Type.String(), rt.Type.String()))
			return unknownAttr()
		}
	...
	return attributeFn(func(ev *evaluationContext) *QualifiedAttribute {
		...
		for _, e := range p0.Expressions {
			...
			if !lastType.Equals(etType.Type) {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("expression list expected %s. Received: %s", lastType.String(), etType.Type.String()))
				return unknownAttr()
			}
		}
...
func (t *typeVisitor) VisitExpressionSelect(p0 *tree.ExpressionSelect) any {
	return attributeFn(func(ev *evaluationContext) *QualifiedAttribute {
		...
		if len(shape) != 1 && !p0.IsExists {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, errors.New("subquery must return exactly one column"))
			return unknownAttr()
		}
...
func (t *typeVisitor) VisitExpressionStringCompare(p0 *tree.ExpressionStringCompare) any {
	return attributeFn(func(ev *evaluationContext) *QualifiedAttribute {
		...
		if !at.Type.Equals(bt.Type) {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("string comparison expression expected %s. Received: %s", at.Type.String(), bt.Type.String()))
			return unknownAttr()
		}
		...
		if p0.Escape != nil {
			...
			if !et.Type.Equals(types.TextType) {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("string comparison escape expected text. Received: %s", et.Type.String()))
				return unknownAttr()
			}
		}
...
func (t *typeVisitor) VisitExpressionUnary(p0 *tree.ExpressionUnary) any {
	return attributeFn(func(ev *evaluationContext) *QualifiedAttribute {
		...
		if !ot.Type.Equals(types.IntType) {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeType, fmt.Errorf("unary expression expected int. Received: %s", ot.Type.String()))
			return unknownAttr()
		}
...
func (t *typeVisitor) VisitInsertCore(p0 *tree.InsertCore) any {
	return returnFunc(func(e *evaluationContext) *Relation {
		...
		if !ok {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("table %s not found", p0.Table))
			return newRelation()
		}

		_, ok = t.ctes[p0.Table]
		if ok {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("cannot insert into common table expression %s", p0.Table))
			return newRelation()
		}
		...
		for _, col := range p0.Columns {
			...
			if !ok {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("column %s not found", col))
				return newRelation()
			}
		}
		...
		for _, row := range p0.Values {
			if len(row) != len(p0.Columns) {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, errors.New("mismatched column/value count"))
				return newRelation()
			}
			...
			for i, val := range row {
				...
				if !ok {
					t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("unknown column %s", p0.Columns[i]))
					return newRelation()
				}

				if !expectedAttr.Type.Equals(attr.Type) {
					t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("%s: type mismatch for column %s", ErrInvalidType.Error(), p0.Columns[i]))
					return newRelation()
				}
			}
		}
...
func (t *typeVisitor) VisitUpdateCore(p0 *tree.UpdateCore) any {
	return returnFunc(func(e *evaluationContext) *Relation {
		...
		if !ok {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("unknown table %s", p0.QualifiedTableName.TableName))
			return newRelation()
		}

		_, ok = t.ctes[p0.QualifiedTableName.TableName]
		if ok {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("cannot update common table expression %s", p0.QualifiedTableName.TableName))
			return newRelation()
		}
		...
		if p0.Where != nil {
			...
			if !whereType.Type.Equals(types.BoolType) {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("%s: where clause must be boolean. Got %s", ErrInvalidType.Error(), whereType.Type.String()))
				return newRelation()
			}
		}
...
func (t *typeVisitor) VisitDeleteCore(p0 *tree.DeleteCore) any {
	return returnFunc(func(e *evaluationContext) *Relation {
		...
		_, ok = t.ctes[p0.QualifiedTableName.TableName]
		if ok {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("cannot delete from common table expression %s", p0.QualifiedTableName.TableName))
			return newRelation()
		}
	...
	return returnFunc(func(e *evaluationContext) *Relation {
		...
		for _, sel := range p0.SimpleSelects[1:] {
			...
			if len(shape) != len(expectedShape) {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("%s: compound selects must return the same number of columns. Expected %d. Received: %d", ErrCompoundShape.Error(), len(expectedShape), len(shape)))
				return newRelation()
			}

			for i, col := range shape {
				if !col.Equals(expectedShape[i]) {
					t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("%s: compound selects must return the same types: Expected %s Received: %s", ErrCompoundShape.Error(), expectedShape[i].Name, col.Name))
					return newRelation()
				}
			}
		}
	...
	return returnFunc(func(e *evaluationContext) *Relation {
		...
		if p0.Where != nil {
			...
			if !a.Attribute.Type.Equals(types.BoolType) {
				t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("%s: where clause must be boolean", ErrInvalidType.Error()))
				return newRelation()
			}
		}
...
func (t *typeVisitor) VisitResultColumnTable(p0 *tree.ResultColumnTable) any {
	return resultFunc(func(e *evaluationContext, r *Relation) {
		...
		if !ok {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, fmt.Errorf("table %s not found", p0.TableName))
			return
		}
...
func (t *typeVisitor) VisitReturningClauseColumn(p0 *tree.ReturningClauseColumn) any {
	return resultFunc(func(e *evaluationContext, r *Relation) {
		...
		if p0.Expression == nil {
			t.ErrorListener.NodeErr(p0.GetNode(), parseTypes.ParseErrorTypeSemantic, errors.New("invalid returning clause"))
			return
		}
