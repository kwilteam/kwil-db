...
// Trigger migration
func TriggerMigration(ctx context.Context, t *testing.T, netops MigrationOpsDsl, chainID string) {
	t.Log("Executing migration trigger specification")

	// Trigger migration"
	txHash, err := netops.TriggerMigration(ctx, big.NewInt(5), big.NewInt(200), chainID)
...
// Genesis state
func InstallGenesisState(ctx context.Context, t *testing.T, netops MigrationOpsDsl, rootDir string, numNodes int, adminAddresses []string) {
	...
	require.Eventually(t, func() bool {
		inMigration, data, err = netops.GenesisState(ctx)
		require.NoError(t, err)
		return inMigration
	...
	require.NotNil(t, metadata.GenesisSnapshot)
	...
	downloadGenesisSnapshot(ctx, t, netops, tempSnapshotFile, metadata.GenesisSnapshot.Height, metadata.GenesisSnapshot.ChunkCount)
