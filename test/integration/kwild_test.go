...
func TestKwildValidatorRemoval(t *testing.T) {
	...
	for _, driverType := range testDrivers {
		...
		t.Run(driverType+"_driver", func(t *testing.T) {
			...
			targetPubKey := helper.NodePrivateKey("node3").Public().Bytes()
...
func TestKwildNetworkHardfork(t *testing.T) {
	...
	t.Run(driverType+"_driver", func(t *testing.T) {
		...
		dr0 := node0Driver.(*driver.KwildClientDriver)
		cl := dr0.Client().(*client.Client)
		n0AcctStatus, err := cl.GetAccount(ctx, cl.Signer.Identity(), types.AccountStatusLatest)
		if err != nil {
			t.Fatal(err)
		}
		noopTx, err := types.CreateTransaction(&noopPayload{}, helper.ChainID(), uint64(n0AcctStatus.Nonce)+1)
		if err != nil {
			t.Fatal(err)
		}
...
func (a *noopPayload) Type() types.PayloadType {
	return "noop"
}
...
func TestKwildPrivateNetworks(t *testing.T) {
	...
	t.Run("cli_driver", func(t *testing.T) {
		...
		node1PubKey := node1PrivKey.Public().Bytes()
		...
		node2PubKey := node2PrivKey.Public().Bytes()
