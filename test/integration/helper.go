...
func (r *IntHelper) MigrationSetup(ctx context.Context) string {
	...
	for i := range r.cfg.NNonValidator + r.cfg.NValidator {
		...
		genCfg, err := config.LoadGenesisConfig(filepath.Join(newNodeDir, "genesis.json"))
		require.NoError(r.t, err)
...
func (r *IntHelper) ExtractPrivateKeys(home string) {
	regexPath := filepath.Join(home, "*", "nodekey.json")
	...
	for idx, file := range files {
		privKey, err := key.LoadNodeKey(file)
		require.NoError(r.t, err, "failed to read private key")

		name := fmt.Sprintf("node%d", idx)
		r.privateKeys[name] = privKey
	}
}
...
func (r *IntHelper) EnableStatesync(ctx context.Context, homeDir string, node string, snapshotProviders []string) {
	...
	for _, provider := range snapshotProviders {
		...
		_, rpcURL, err := utils.KwildJSONRPCEndpoints(ctr, ctx)
	...
	cfg.StateSync.TrustedProviders = providers
...
func decodePrivateKey(pkey string, keyType crypto.KeyType) (crypto.PrivateKey, error) {
	privB, err := hex.DecodeString(pkey)
	if err != nil {
		return nil, fmt.Errorf("error decoding private key: %v", err)
	}

	return crypto.UnmarshalPrivateKey(privB, keyType)
}
...
func (r *IntHelper) getUserDriver(ctx context.Context, nodeName string, driverType string, deployer *ethdeployer.Deployer, chainID string) KwilIntDriver {
	...
	cometBftURL, _, err := utils.KwildJSONRPCEndpoints(ctr, ctx)
...
func (r *IntHelper) NodePrivateKey(name string) crypto.PrivateKey {
	return r.privateKeys[name]
}

func (r *IntHelper) NodeKeys() map[string]crypto.PrivateKey {
...
func (r *IntHelper) NodeIDs() map[string]string {
	...
	for name, key := range r.privateKeys {
		nodeID := peers.NodeIDFromPubKey(key.Public())
		nodeIDs[name] = nodeID
	}
	return nodeIDs
}
