...
// Options can be used to customize the connection.
func Dial(target string, opts ...ClientOption) (*Client, error) {
	...
	case "http", "https":
	default:
		return nil, fmt.Errorf("url scheme must be either http or https. received: %s", u.Scheme)
	}
...
// WrapConn wraps an existing http connection
func WrapConn(conn *http.Client) *Client {
	return &Client{
		conn: conn,
	}
}
